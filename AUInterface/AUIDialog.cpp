...
bool AUIDialog::InitFrameXML(AXMLItem *pItem)
{
	...
	XMLGET_INT_VALUE(pItem, _AL("FadeMode"), m_nFadeMode)
...
bool AUIDialog::Save(const char *pszFilename)
{
	...
	if( len >= 4 && stricmp(pszFilename + len - 4, ".xml") == 0 )
	{
		...
		XMLSET_INT_VALUE(pXMLItemDialog, _AL("FadeMode"), m_nFadeMode, 0)
...
void AUIDialog::Show(bool bShow, bool bModal, bool bActive, bool bFocusLock)
{
...
bool AUIDialog::Render(void)
{
	...
	if (m_nFadeMode > 0 && m_nWholeAlpha != 255) {

		m_nWholeAlpha = ((255 * (GetTickCount() - m_nFadeTimer) / m_nFadeMode));
		if (m_nWholeAlpha > 255)
			m_nWholeAlpha = 255;
	}
...
bool AUIDialog::SetProperty(const char *pszPropertyName, PAUIOBJECT_SETPROPERTY Property)
{
	...
	else if( 0 == strcmpi(pszPropertyName, "Width") )
	{
		...
		if( m_pAUIFrameDlg )
		{
			...
			m_nWidth = max(m_nWidth, (int)s.cx);
		}
	...
	else if( 0 == strcmpi(pszPropertyName, "Height") )
	{
		...
		{
			SIZE s = m_pAUIFrameDlg->GetSizeLimit();
			m_nHeight = max(m_nHeight, (int)s.cy);
		}
...
#else
	// Access the raw D3D surface directly
	bool bval = suf.Init(m_pA3DDevice, pTarget->GetUIRenderTarget()->GetTargetSurface(), "");
#endif
	if (bval)
	{
		...
		// copy
		A3DRECT rcUpdate(0,0,m_nWidth,m_nHeight);
		if (!pTexture->UpdateFromSurface(&suf, rcUpdate))
		{
			A3DRELEASE(pTarget);
			return false;
		}
