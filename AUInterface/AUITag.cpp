...
AUIRichTextPrinter::AUIRichTextPrinter()
{
	...
	RegisterTagtypeBuiltIn(AUITextTagCtrlChannel());			//^p����^p ���� ^p^ffff00����^p
	RegisterTagtypeBuiltIn(AUITextTagCtrlColor());				//^ffffff

	// edit box items
	RegisterTagtypeBuiltIn(AUITextTagItem());					//��
...
void AUIRichTextPrinter::RegisterTagtype(const AUITextTag& tg)
{
	AUITAG_NODE* pNode = const_cast<AUITextTag&>(tg).CreateNode(this);
	ASSERT(pNode);
	if (m_mpNodesBuildIn.find(tg.GetClassName()) == m_mpNodesBuildIn.end())
		m_mpNodesCustom[tg.GetClassName()] = pNode;
}

void AUIRichTextPrinter::RegisterTagtypeBuiltIn(const AUITextTag& tg)
{
	AUITAG_NODE* pNode = const_cast<AUITextTag&>(tg).CreateNode(this);
	ASSERT(pNode);
...
bool AUIRichTextPrinter::UnRegisterTagType(const AUITextTag& tg)
{
...
const AUITextTag* AUIRichTextPrinter::RenderPlainText(TAG_OP_IN_PARAMS& inParams, TAG_RESOLVER_CONTEXT& context, const TAG_OP_OUT_PARAMS& outParams) const
{
	...
	while (*p != '\0')
	{
		...
		if (IsNewLineNeed(inParams, context, nTextWidth + nCurWordWidth))
		{
			...
			else if (iTextEOF > context.index) // ������ѭ��
				break;
		}
