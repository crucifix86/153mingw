			...
			for( i = nStart; i <= nEnd; i++ )
			{
				...
				if( m_bAutoWrap )
				{
					float fScaleY = min(float(fTextH * m_Item[i].nLines),
						float(rcText.bottom - y)) / float(m_pA2DSprite[AUILISTBOX_RECT_ITEM]->GetHeight());
					m_pA2DSprite[AUILISTBOX_RECT_ITEM]->SetScaleY(fScaleY);
				}
				else
				{
		...
		if( m_Item.size() > 0 && m_pA2DSprite[AUILISTBOX_RECT_HILIGHT] )
		{
			...
			for( i = nStart; i <= nEnd; i++ )
			{
					...
					|| (!m_bMultipleSelection && i == m_nCurSel) )
				{
					...
					{
						float fScaleY = min(float(fTextH * m_Item[i].nLines),
							float(rcText.bottom - y)) / float(m_pA2DSprite[AUILISTBOX_RECT_HILIGHT]->GetHeight());
						m_pA2DSprite[AUILISTBOX_RECT_HILIGHT]->SetScaleY(fScaleY);
					}
					else
					{
		...
		for( i = nStart; i <= nEnd; i++ )
		{
			...
			if( m_nNumColumns > 1 )
			{
				...
				for( int j = 0; j < nNumCols; j++ )
				{
					if (m_bRenderHScroll)
					{
						int l = max(rcText.left, (int)(x - m_nHBarPos));
						int r = max(rcText.left, (int)(l + m_nColWidth[j] * fWindowScale));
						rc.SetRect(l, y, r, min((int)(y + fTextH * m_Item[i].nLines), rcText.bottom));
					}
					else
					{
						rc.SetRect(x, y, x + m_nColWidth[j] * fWindowScale, min((int)(y + fTextH * m_Item[i].nLines), rcText.bottom));
					}
