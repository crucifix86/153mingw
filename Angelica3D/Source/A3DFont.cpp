...
bool A3DFont::LoadCharTable(const char* szFile)
{
	...
	if (!File.Open(szFullFileName, AFILE_OPENEXIST | AFILE_BINARY))
	{
		File.Close();
	...
	AddCharsToTable(pNewChars, iNewCharCnt, true);
...
A3DFont::TEXTURESLOT* A3DFont::AllocTextureSlot()
{
	...
	//	Create texture
	if (!pSlot->pTexture->Create(m_pA3DDevice, m_iTexWidth, m_iTexHeight, A3DFMT_A4R4G4B4, 1))
	{
		FreeTextureSlot(pSlot);
...
void A3DFont::SelectA3DFont()
{
	m_pA3DFontMan->BeginGetTextExtent(m_hA3DFont);
}

void A3DFont::UnSelectFont()
{
	m_pA3DFontMan->EndGetTextExtent();
}

//	Add new characters to char table
bool A3DFont::AddCharsToTable(const ACHAR* szText, int iNumChar, bool bSelectFont)
{
	if (m_bNoRender)
		return true;
...
bool A3DFont::PrintCharsOnTextures(APtrArray<CHARPROP*>& aChars)
{
	...
	for (i=0; i < iNumChar; i++)
	{
		...
		if (!pSlot->pTexture->LockRect(NULL, (void**) &pTexBuf, &iTexPitch, 0))
		{
...
bool A3DFont::TextOut(int x, int y, const ACHAR* szText, A3DCOLOR color, 
					  int iHeight/* 0 */, float z/* 0.0f */, bool bBorder/* false */, bool bSelectFont/*true*/)
{
	...
	AddCharsToTable(szText, iLen, bSelectFont);
					   ...
					   int iHeight/* 0 */, float z/* 0.0f */)
{
	...
	//	Add unregistered characters
	AddCharsToTable(szText, iLen, true);
						...
						int iHeight, A3DCOLOR color)
{
	...
	//	Add unregistered characters
	AddCharsToTable(szText, iTextLen, true);
...
bool A3DFont::FlushOutputBuf(int iSlot)
{
	...
	if (!m_pA3DStream->LockVertexBuffer(0, dwLockSize, (BYTE**) &pDestBuf, 0))
	{
	...
	//	Unlock stream
	m_pA3DStream->UnlockVertexBuffer();
