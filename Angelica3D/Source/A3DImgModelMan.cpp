...
static int _cdecl _IMActionSortCompareFunc(const void *arg1, const void *arg2)
{
...
static int _cdecl _IMMarkPtSortCompareFunc(const void *arg1, const void *arg2)
{
													  ...
													  char* szTexPath)
{
	...
	while (pFile->PeekNextToken(true))
	{
		...
		if (!stricmp(pFile->m_szToken, "texturenumber"))
		{
...
bool A3DImageModelMan::ParseActionSectionInGroupFile(MODELGROUP* pGroup, AScriptFile* pFile)
{
	...
	while (pFile->PeekNextToken(true))
	{
		...
		if (!stricmp(pFile->m_szToken, "actionnumber"))
		{
		...
		else
		{
			...
			//	Action name
			pFile->GetNextToken(false);
			strcpy(pAct->szName, pFile->m_szToken);
...
bool A3DImageModelMan::ParseCellSectionInGroupFile(MODELGROUP* pGroup, AScriptFile* pFile)
{
	...
	while (pFile->PeekNextToken(true))
	{
		...
		if (!stricmp(pFile->m_szToken, "cellnumber"))
		{
													...
													char* szModelPath)
{
	...
	while (pFile->PeekNextToken(true))
	{
		...
		if (!stricmp(pFile->m_szToken, "modelnumber"))
		{
...
bool A3DImageModelMan::ParseTexInfoSectionInGroupFile(MODELGROUP* pGroup, AScriptFile* pFile)
{
	...
	while (pFile->PeekNextToken(true))
	{
		...
		if (!stricmp(pFile->m_szToken, "origin") ||
			!stricmp(pFile->m_szToken, "scale"))
		{
			//	Don't handle these
		}
...
bool A3DImageModelMan::ParseMarkPtSectionInGroupFile(MODELGROUP* pGroup, AScriptFile* pFile)
{
	...
	while (pFile->PeekNextToken(true))
	{
		...
		if (!stricmp(pFile->m_szToken, "marknumber"))
		{
		...
		else
		{
			...
			//	Name
			pFile->GetNextToken(false);
			strcpy(pMarkPt->szName, pFile->m_szToken);
...
A3DImageModelMan::TEXTURE* A3DImageModelMan::AllocTextureSlot(MODELGROUP* pGroup, int iCount)
{
	...
	memset(pSlot->aRModels, 0, sizeof (16 * sizeof (A3DImgModel*)));
