...
bool A3DTerrain2::Load(const char* szFileName, float cx, float cz, bool bMultiThread/* false */)
{
	...
	{
		A3DTerrain2LoaderB* pLoader = new A3DTerrain2LoaderB(this);
		if (!pLoader->Init(szFileName))
		{
			g_A3DErrLog.Log("A3DTerrain2::Load, Failed to initialize terrain2 laoder");
			return false;
	...
	m_pTrnLoader = CreateLoader(T2Header.iLoaderID);
	if (m_pTrnLoader)
	{
		if (!m_pTrnLoader->Init(szFileName))
		{
			g_A3DErrLog.Log("A3DTerrain2::Load, Failed to initialize terrain2 loader");
			return false;
		}
	}
	else
...
A3DTerrain2Loader* A3DTerrain2::CreateLoader(int iType)
{
	//	Create terrain loader
	if (iType == A3DTerrain2Loader::LOADER_A)
	{
		A3DTerrain2LoaderA* pLoader = new A3DTerrain2LoaderA(this);
		return pLoader;
	}
	else if (iType == A3DTerrain2Loader::LOADER_B)
	{
		A3DTerrain2LoaderB* pLoader = new A3DTerrain2LoaderB(this);
		return pLoader;
	}
...
//	Render routine
bool A3DTerrain2::Render(A3DViewport* pViewport, bool bShadow, A3DRenderTarget* pTerrainColorRT, float fZBias, float fShadowLum, bool bHDRLightMap)
{
	...
	//	Do render
	if (!m_pCurRender->Render(pViewport, bShadow, pTerrainColorRT, fZBias, fShadowLum, bHDRLightMap))
	{
...
void A3DTerrain2::UpdateBlocksLOD(const A3DVECTOR3& vCenter)
{
	...
	if (m_LODType == LOD_USEHEIGHT)
	{
		...
		if (iDecLevel)
		{
			for (int r = 0; r < aBlocks.GetSize(); r++)
			{
