					...
					if (bHit)
					{
						...
						{		
							...
							else
							{
								...
								if( pParticle->pModel->FindGroupActionByName("[���]", NULL) )
								{
									pParticle->pModel->PlayActionByName("[���]", true);
								}
							}
						}
...
bool A3DParticleSystem::RenderStandardParticles(A3DViewport * pCurrentViewport)
{
	...
	for(int id=0; id<m_nNumParticles; id++)
	{
		...
		case A3DPARTICLE_STANDARD_PARTICLE_CONSTANT:
			if( m_StandardParticleType == A3DPARTICLE_STANDARD_PARTICLE_FACING )
			{
				...
				if( pCommonParticle->vBubbleAmplitude != 0.0f )
				{
					...
					FLOAT vSin = g_pA3DMathUtility->SIN(pCommonParticle->vBubblePhase);
			...
			{
				FLOAT vSin, vCos;
				vSin = g_pA3DMathUtility->SIN(pCommonParticle->vSpinPhase);
				vCos = g_pA3DMathUtility->COS(pCommonParticle->vSpinPhase);
		...
		if( m_nIndexCount >= m_nMaxIndexCount || m_nVertCount >= m_nMaxVertCount )
		{
			...
			m_pA3DStream->SetVerts((LPBYTE)m_pVertexBuffer, m_nVertCount);
			m_pA3DStream->Appear();
...
bool A3DParticleSystem::RenderObjectFragmentParticles(A3DViewport * pCurrentViewport)
{
	...
	for(int id=0; id<m_nNumParticles; id++)
	{
			...
			m_pnIndexCount[nTextureID] + pParticle->nIndexCount > m_nMaxIndexCount )
		{
			...
			m_pA3DStream->SetVerts((LPBYTE)m_ppVertexBuffers[nTextureID], m_pnVertCount[nTextureID]);
			m_pA3DStream->SetIndices(m_ppIndexBuffers[nTextureID], m_pnIndexCount[nTextureID]);
			m_pA3DStream->Appear();
...
NEXTPARTICLE:
	...
	for(i=0; i<m_nTextureNum; i++)
	{
		if( m_pnVertCount[i] )
		{
			...
			m_pA3DDevice->SetDestAlpha(m_DestBlend);
			m_pA3DDevice->SetLightingEnable(false);
			m_pA3DStream->SetVerts((LPBYTE)m_ppVertexBuffers[i], m_pnVertCount[i]);
			m_pA3DStream->SetIndices(m_ppIndexBuffers[i], m_pnIndexCount[i]);
			m_pA3DStream->Appear();
...
bool A3DParticleSystem::UpdateParentTM(const A3DMATRIX4& matParent)
{
