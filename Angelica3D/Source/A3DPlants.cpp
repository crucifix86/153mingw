...
bool A3DPlants::AddTreeType(char * szTreeFile, char * szSpriteFile, int nDegNum, int nTexRow, int nTexCol, FLOAT vTexDU, FLOAT vTexDV, DWORD * pTreeHandle, const A3DVECTOR3& vecScale)
{
	A3DFrame * pTreeFrame;
...
A3DPlants::TREETYPE * A3DPlants::FindTreeType(char * szTreeFile)
{
	...
	{
		if( stricmp(szTreeFile, m_pTreeTypes[i].szTreeFile) == 0 )
...
bool A3DPlants::AddGrassType(char * szGrassFile, DWORD * pGrassHandle, const A3DVECTOR3& vecScale)
{
...
A3DPlants::GRASSTYPE * A3DPlants::FindGrassType(char * szGrassFile)
{
	...
	{
		if( stricmp(szGrassFile, m_pGrassTypes[i].szGrassFile) == 0 )
