...
public:
	...
	{
		typename CachedItemMap::iterator it = m_ItemMap.find(strName);
	...
	T RemoveLRUItem()
	{
		const AString* pstrLRU = NULL;
		PITEM pItem = NULL;
		int nLastAccess = m_nHitCount;
		T _ret = 0;

		for (typename CachedItemMap::iterator it = m_ItemMap.begin(); it != m_ItemMap.end(); ++it)
		{
	...
	T RemoveHead()
	{
		typename CachedItemMap::iterator it = m_ItemMap.begin();
