...
void FWDialogParam::ShowProp()
{
	...
	{
	case GFX_VALUE_INT:
		tmp1.Format(_AL("%s：(%d ～ %d)"), pParam->GetName(), (int)pParam->GetMin(), (int)pParam->GetMax());
		tmp2.Format(_AL("%d"), (int)pParam->GetVal());
		break;
	case GFX_VALUE_FLOAT:
		tmp1.Format(_AL("%s：(%.3f ～ %.3f)"), pParam->GetName(), (float)pParam->GetMin(), (float)pParam->GetMax());
		tmp2.Format(_AL("%.3f"), (float)pParam->GetVal());
		break;
	case GFX_VALUE_STRING:
		tmp1.Format(_AL("%s："), pParam->GetName());
