...
bool CQBrush::AddBrushBevels(CConvexHullData * pCHData)
{
	Release();
	assert(pCHData!= NULL);
	m_pCHData = pCHData;
...
bool CQBrush::_FlushCH()
{
	if (0 == m_pCHData || m_pCHData->IsEmpty())
	{
		assert(m_pCHData != NULL && !m_pCHData->IsEmpty());
		return false;
	}
	if (0 != m_nQPlane)
	{
	...
	for (int i = 0; i < nFace; ++i, ++m_nQPlane)
	{
