...
char* AString::AllocBuffer(int iLen)
{
	...
	pData->m_pLock = new std::mutex();
	return pBuf + sizeof(s_STRINGDATA);
}

//	Free string data buffer
void AString::FreeBuffer(s_STRINGDATA* pStrData, bool& deleteLock)
{
...
AString::AString(const AString& str)
{
	...
	std::lock_guard<std::mutex> lock(*pSrcData->m_pLock);
...
AString::~AString()
{
	...
	std::mutex* pLock = NULL;
	{

		if (m_pStr != m_pEmptyStr)
		{
			pLock = pData->m_pLock;
			std::lock_guard<std::mutex> lock(*pData->m_pLock);

			if (pData->iRefs == -1)	//	Buffer is being locked
				pData->iRefs = 1;

			// dont attempt to remove empty string
			if (m_pStr != m_pEmptyStr)
				FreeBuffer(pData, deleteLock);
