...
bool EventSystem::Init(int maxchannels, INITFLAGS flags, void *extradriverdata)
{
	...
	{
		m_pLogManager->GetEngineLog()->Write(LOG_ERROR, "EventSystem::Init, System_Create, ����FMOD_Systemʧ��");
		m_bFailedInit = true;
		return false;
	}
	if(FMOD_OK != m_pSystem->init(maxchannels, flags, extradriverdata))
	{
		m_pLogManager->GetEngineLog()->Write(LOG_ERROR, "EventSystem::Init, System->init, ��Ƶ�����ʼ��ʧ��");
		m_bFailedInit = true;
		return false;	
	}
	if(FMOD_OK != m_pSystem->setFileSystem(afile_open, afile_close, afile_read, afile_seek, 0, 0, 2048))
	{
		m_pLogManager->GetEngineLog()->Write(LOG_ERROR, "EventSystem::Init, System->setFileSystem, ����FILE�ص�����ʧ��");
		m_bFailedInit = true;
		return false;
	}
	...
	{
		m_pLogManager->GetEngineLog()->Write(LOG_ERROR, "EventSystem::Init, m_pEffectFactory->init, Ч����������ʼ��ʧ��");
		m_bFailedInit = true;
		return false;
	}
	...
	{
		m_pLogManager->GetEngineLog()->Write(LOG_ERROR, "EventSystem::Init, AudioInstanceManager->init, ��Ƶ�ļ�ʵ����������ʼ��ʧ��");
		m_bFailedInit = true;
		return false;
	}
...
/* FOR DEBUG
	...
	m_pLogManager->GetEngineLog()->Write(LOG_ERROR, "�ڴ�ʹ�ã�%.2f M\tCPUռ�ã�%.2f %%, DSP CPU��%.2f %%, Channels��%d", uiBits/1000000.0f, fTotal, fDsp, GetPlayingChannelsNum());
	...
	m_pLogManager->GetEngineLog()->Write(LOG_ERROR, "�ڴ���ϸ��������DSP��%.2f M\tChannel��%.2f M\tChannel Group��%.2f M\tSound��%.2f M", uiMemDsp/1000000.0f, uiMemChannel/1000000.0f, uiMemChannelGroup/1000000.0f, uiMemSound/1000000.0f);
...
bool EventSystem::LoadProjects()
{
	...
	if(m_bFailedInit)
	{
		m_pLogManager->GetEngineLog()->Write(LOG_ERROR, "EventSystem::LoadProjects ��Ƶϵͳ��ʼ��ʧ�ܣ���ֹ������Ƶ���̡�");
		return false;
	}
...
#endif
	{
		m_pLogManager->GetEngineLog()->Write(LOG_ERROR, "EventSystem::LoadProjects �޷����ļ�%s", szPath);
		return false;
	}
...
#else
	char* pBuf = (char *)a_malloc(dwLength + 1);
#endif
