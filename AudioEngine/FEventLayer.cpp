...
bool EventLayer::Load(AFileImage* pFile)
{
	...
	for (int i=0; i<iSoundInstanceNum; ++i)
	{
		...
		{
			m_pEventProject->GetEventSystem()->GetLogManager()->GetEngineLog()->Write(LOG_ERROR, "EventLayer::Load ����SoundInstanceTemplate:%sʧ��", pSoundInstanceTemplate->GetSoundDef()->GetName());
			delete pSoundInstanceTemplate;
			return false;
		}
	}
	...
	for (int i=0; i<iEffectNum; ++i)
	{
		...
		{
			m_pEventProject->GetEventSystem()->GetLogManager()->GetEngineLog()->Write(LOG_ERROR, "EventLayer::Load ����Effect:%sʧ��", pEffect->GetName());
			return false;
		}
	}
	return true;
...
bool EventLayer::Save(AFile* pFile)
{
	...
	for (; itSoundIns!=m_listSoundInstanceTemplate.end(); ++itSoundIns)
	{
		...
		{
			m_pEventProject->GetEventSystem()->GetLogManager()->GetEngineLog()->Write(LOG_ERROR, "EventLayer::Save ����SoundInstanceTemplate:%sʧ��", pSoundInstanceTemplate->GetSoundDef()->GetName());
			return false;
		}
	}
	...
	for (; itEffect!=m_listEffect.end(); ++itEffect)
	{
		...
		{
			m_pEventProject->GetEventSystem()->GetLogManager()->GetEngineLog()->Write(LOG_ERROR, "EventLayer::Save ����Effect:%sʧ��", pEffect->GetName());
			return false;
		}
	}
	return true;
...
bool EventLayer::LoadXML(TiXmlElement* root, bool bPreset /*= false*/)
{
	...
	if(!bPreset)
	{
		...
		for (int i=0; i<iSoundInstanceNum; ++i)
		{
			...
			{
				m_pEventProject->GetEventSystem()->GetLogManager()->GetEngineLog()->Write(LOG_ERROR, "EventLayer::LoadXML ����SoundInstanceTemplate:%sʧ��", pSoundInstanceTemplate->GetSoundDef()->GetName());
				delete pSoundInstanceTemplate;
				return false;
			}
	...
	{		
		...
		{
			m_pEventProject->GetEventSystem()->GetLogManager()->GetEngineLog()->Write(LOG_ERROR, "EventLayer::LoadXML ����Effect:%sʧ��", pEffect->GetName());
			return false;
		}
...
bool EventLayer::SaveXML(TiXmlElement* pParent, bool bPreset /*= false*/)
{
	...
	if(!bPreset)
	{
		...
		for (; itSoundIns!=m_listSoundInstanceTemplate.end(); ++itSoundIns)
		{
			...
			{
				m_pEventProject->GetEventSystem()->GetLogManager()->GetEngineLog()->Write(LOG_ERROR, "EventLayer::SaveXML ����SoundInstanceTemplate:%sʧ��", pSoundInstanceTemplate->GetSoundDef()->GetName());
				return false;
			}
		}
	}
	...
	for (; itEffect!=m_listEffect.end(); ++itEffect)
	{
		...
		{
			m_pEventProject->GetEventSystem()->GetLogManager()->GetEngineLog()->Write(LOG_ERROR, "EventLayer::SaveXML ����Effect:%sʧ��", pEffect->GetName());
			return false;
		}
	}
	return true;
