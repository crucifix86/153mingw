...
bool CLuaThread::_Resume(int nargs, abase::vector<CScriptValue>& results, int rt_start)
{
	...
	{
		LuaBind::ReportError(NULL, __FILE__, (char*)__FUNCTION__, __LINE__, "cannot resume error or dead thread");
		return false;
	}
	...
	{
		LuaBind::ReportError(m_pThreadVM, __FILE__, (char*)__FUNCTION__, __LINE__, "CLuaThread::_Resume failed");
		m_Status = LT_ERROR;
		return false;
	}
