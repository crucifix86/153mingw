...
FT_BitmapGlyph FTFont::RenderChar(UINT c, bool bRenderAsOutline)
{
	...
	if (0 == index)
	{
		// Use a question mark as fallback character (originally was Chinese character)
...
BOOL FTFont::TextOutInternal(int nXStart, int nYStart, const wchar_t *lpString, int cbString, BYTE * pBuffer, int nWidth, int nHeight, bool bRenderAsOutline)
{
	...
	{
		ZeroMemory(&pBuffer[y * nWidth + nXStart], min((int)size.cx, nWidth - nXStart));
	}
	...
	for (int i = 0; i < cbString; i++)
	{
		...
		if (y < 0) 
			startJ = -y;
		for (int j = startJ; j < min((int)ftGlyph->bitmap.rows, nHeight - y); j++)
		{
			for (int i = 0; i < min((int)ftGlyph->bitmap.width, nWidth - x); i++)
			{
...
BOOL FTFont::DrawCharInternal(wchar_t ch, BYTE* pBuffer, int nWidth, int nHeight, bool bRenderAsOutline)
{
	...
	int min_row = min((int)ftGlyph->bitmap.rows, nHeight);
	int min_col = min((int)ftGlyph->bitmap.width, nWidth);
