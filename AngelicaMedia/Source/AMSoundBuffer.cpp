...
bool AMSoundBuffer::Play(bool bLoop)
{
	...
	if( m_nState != AMSBSTATE_PAUSED )
	{
		AMConfig* pAMConfig = m_pAMSoundEngine->GetAMEngine()->GetAMConfig();
		if( m_pImmEffect && pAMConfig->GetFlagForceFeedback() )
			m_pImmEffect->Start();
	}
...
bool AMSoundBuffer::CheckEnd()
{
	...
	if( m_nState == AMSBSTATE_PLAYING && !m_bLooping )
	{
		...
		{
			// overtime check
			if( ACounter::GetMilliSecondNow() - m_dwStartTime > DWORD((long long)(m_bufferDesc.dwBufferBytes) * 1000 / m_bufferDesc.wfxFormat.nAvgBytesPerSec + 100) )
				Stop();
		}
	}
...
bool AMSoundBuffer::LoadImmEffect()
{
	...
	if (m_pAMSoundEngine->GetAMEngine()->GetAMImmWrapper())
	{
		...
		if (!af_GetFileTitle(m_strFileName, szTitle, MAX_PATH))
			return false;
