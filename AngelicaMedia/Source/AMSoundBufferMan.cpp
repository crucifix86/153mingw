...
{
	::DeleteCriticalSection(&m_cs3DLoad);
	::DeleteCriticalSection(&m_cs2DLoad);
}

//	Initialize object
bool AMSoundBufferMan::Init(AMSoundEngine* pAMSoundEngine)
{
	::InitializeCriticalSection(&m_cs3DLoad);
	::InitializeCriticalSection(&m_cs2DLoad);
...
bool AMSoundBufferMan::Play2DAutoSound(const char* szFile, bool bImmEffect/* false */)
{
	...
	if (bImmEffect)
		Buf.pSoundBuffer->LoadImmEffect();
