...
bool AMSoundStream::Play(const char * pszStreamFile)
{
	if( pszStreamFile && 0 != stricmp(m_szStreamFile, pszStreamFile) )
	{
...
{
	if (pFile && dwDataLen && 0 != stricmp(m_szStreamFile, pFile->GetFileName()))
	{
		// Now we need to change a stream file;
		ReleaseResource();
		if (!LoadStreamFromOggData(pFile, dwDataLen))
