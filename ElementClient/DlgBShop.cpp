...
void CDlgBShop::OnSysAuctionAccount(void* pData)
{
	...
	if(ERR_SUCCESS == p->retcode)
	{
		...
		if (p->bid_ids.empty())
		{
			abase::vector<unsigned int> emptyVec;
			m_Account.Items.swap(emptyVec);
		}
		else
		{
			abase::vector<unsigned int> tempVec(&p->bid_ids[0], &p->bid_ids[0] + p->bid_ids.size());
			m_Account.Items.swap(tempVec);
		}
		SetDirty(true);
	}
}
...
void CDlgBShop::ShowAuctionItem(const GNET::GSysAuctionItem* pItem)
{
	...
	else
	{
		...
		pTxt_FreezeTime->SetText( freezeTime <= 0 ?
			ACString(GetStringFromTable(ASTATE_END == pItem->state ? 5601:5600)) : GetGameUIMan()->GetFormatTime(freezeTime));
		...
		pTxt_LeftTime->SetText( leftTime <= 0 ?
			ACString(GetStringFromTable(ASTATE_END == pItem->state ? 5601:5600)) : GetGameUIMan()->GetFormatTime(leftTime));
...
bool CDlgBShop::LoadItemIcon(AuctionItemInfo* pInfo)
{
	...
	szBigIconFile.Format("surfaces\\����Ʒ\\%s", szFile);
	if(!af_IsFileExist(szBigIconFile))
	{
		return false;
	}
	
	szBigIconFile.Format("����Ʒ\\%s", szFile);
