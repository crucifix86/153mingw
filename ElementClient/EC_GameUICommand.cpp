...
bool CECGameUIMan::OnCommand(const char* szCommand, AUIDialog* pDlg)
{
	...
	if( 0 == stricmp(pDlg->GetName(), "Win_Popmsg") )
		return false;
#if 0  // BBS feature disabled
	if( 0 == stricmp(pDlg->GetName(), "Win_BBSDlg") )
		return m_BBSDlg.OnCommand(szCommand, pDlg);
#endif

#if 0  // Customize feature not fully implemented for MinGW build
	if( GetDialog("Win_Customize") && GetDialog("Win_Customize")->IsShow() && m_CustomizeMgr )
		return m_CustomizeMgr->OnCommand_Customize(szCommand, pDlg);
#endif
...
#if 0  // Mark deletion requires access to protected members - disabled for MinGW build
