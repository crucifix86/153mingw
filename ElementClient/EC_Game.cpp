...
//	 Gameversion
DWORD GAME_VERSION = ((0 << 24) | (1 << 16) | (5 << 8) | 3);
...
int addon_generate_arg(DATA_TYPE type, addon_data & data, int arg_num/*��ʼ�Ĳ�������*/)
{
	return arg_num;
} 
...
BOOL WINAPI MyTerminateProcess(HANDLE hProcess, UINT uExitCode)
{
	*TerminateProcess_Entry = TerminateProcess_Entry_Content;
	::MessageBoxA(NULL, "���棺�����쳣��ֹ!!!!", "Error", MB_OK);
	char* p = 0;
	memcpy(p, "error", 5);
	return TRUE;
}
...
void WINAPI MyExitProcess(UINT uExitCode)
{
	...
	*ExitProcess_Entry = ExitProcess_Entry_Content;
	...
	{
		::MessageBoxA(NULL, "���棺�����쳣�˳�!!!!", "Error", MB_OK);
		char* p = 0;
		memcpy(p, "error", 5);
	}
}
...
bool CECGame::Init(const GAMEINIT& GameInit)
{
	...
	m_pSkinRender1 = (A3DSkinRender*)m_pA3DEngine->GetA3DSkinMan()->GetDefSkinRender();
	...
	if (!CECVipConfig::Instance().Load("configs\\vipconfig.ini"))
	{
		glb_ErrorOutput(ECERR_FAILEDTOCALL, "CECGame::Init", __LINE__);
		return false;
	}
	...
	CreateDirectory (L"reportbugs\\Logs",NULL);
	...
	{
		MessageBoxA(NULL, "�޷�������Ϸ�����ļ���\n�˴�������Ǹ��²���ȫ���£������½��и��»���а汾��֤��", "����", MB_OK | MB_ICONSTOP);
		glb_ErrorOutput(ECERR_FAILEDTOCALL, "CECGame::Init", __LINE__);
		return false;
	}
...
//	m_strAllVersion.Format("%x%x%x0", ELEMENTDATA_VERSION, _task_templ_cur_version, globaldata_getgshop_timestamp());
#ifdef VIP
	m_strAllVersion.Format("%x%x%x%x%x", ELEMENTDATA_VERSION, _task_templ_cur_version, globaldata_getgshop_timestamp(),globaldata_getgshop_timestamp2(), globaldata_getgshop_timestamp3());
#else
...
bool CECGame::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NCACTIVATE:
		if (wParam == FALSE)
		{
			...
			{
				//	���Ӵ��ڵ��£����� true ���޸� wParam ��ʹ DefWndProc ��ʾΪδʧȥ����
				return true;
			}
...
{
	int iRet = -1;
	PROFILE_BEGINSESSION("RenderThread");
