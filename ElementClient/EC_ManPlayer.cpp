...
m_aMMPlayers(0, 256)
{
	...
	// Check the player cache every 2 mintues
	m_CacheCnt.SetPeriod(120000);
...
bool CECPlayerMan::OnLeaveGameWorld()
{
	...
	if (m_PlayerCache.size())
	{
		...
		for (; it != m_PlayerCache.end(); ++it)
		{
			...
			ReleaseElsePlayer(pData->m_pPlayer);
			delete pData;
		}
...
{
	ACSWrapper cspt(&m_csPlayerTab);		// ����Ӹ�������Ϊ�˺����ᱻ��̨�����̵߳��õ�
...
bool CECPlayerMan::TransmitMessage(const ECMSG& Msg)
{
	...
	case MSG_PM_PLAYEREQUIPDATA:
		...
		if (Msg.dwParam2 == EQUIP_DATA)
		{
			...
			cid = pCmd->idPlayer;
			if(pHost->GetPlayerInfo().cid == cid)
			pHost->SetColorName(pCmd->color_name);
		}
...
bool CECPlayerMan::Tick(DWORD dwDeltaTime)
{
	...
	PlayerTable::iterator it = m_PlayerTab.begin();
	int totalChildren = 0;
	CECParallelTickOnlyAnimationManager<CECElsePlayer*> elems;
	CECParallelECMGFXTickAnimationManager<CECModel*> elemECMS;
#if 0
	for (; it != m_PlayerTab.end(); ++it)
	{
		CECElsePlayer* pPlayer = *it.value();
	...
	{
		CECElsePlayer* pPlayer = *it.value();
		elems.AddElem(pPlayer, dwDeltaTime);
		pPlayer->TickBeforeAnimate(dwDeltaTime);
		if (pPlayer->m_pTickedModel)
			totalChildren += pPlayer->m_pTickedModel->GetChildCount();
	}
	...
	{
		CECElsePlayer* pPlayer = *it.value();
		pPlayer->TickAfterAnimate(dwDeltaTime);
		if (pPlayer->m_pTickedModel)
		{
			int size = pPlayer->m_pTickedModel->GetChildCount();
			for (int i = 0; i < size; i++ )
			{
				CECModel* pModel = pPlayer->m_pTickedModel->GetChildModel(i);
				pModel->TickBeforeAnimate(dwDeltaTime);
				elemECMS.AddElem(pModel, dwDeltaTime);
			}
		}
	}
	elemECMS.TickAnimation(dwDeltaTime);
	it = m_PlayerTab.begin();
	for (; it != m_PlayerTab.end(); ++it)
	{
		CECElsePlayer* pPlayer = *it.value();
		if (pPlayer->m_pTickedModel)
		{
			...
			{
				CECModel* pModel = pPlayer->m_pTickedModel->GetChildModel(i);
				pModel->TickAfterAnimate(dwDeltaTime, false, true, true);
			}
		}
	}
#endif
...
bool CECPlayerMan::Render(CECViewport* pViewport)
{
	...
	int idSelTarget = pHost ? pHost->GetSelectedTarget() : 0;
	...
	if (bRenderElse)
	{
		...
		for (; it != m_PlayerTab.end(); ++it)
		{
			...
			else
			{
				...
				else if(pPlayer->GetCharacterID() == idSelTarget)
					pPlayer->RenderHighLight(pViewport);
				else
...
CECElsePlayer* CECPlayerMan::ElsePlayerEnter(const S2C::info_player_1& Info, int iCmd)
{
	...
	// If we are exceeding the cache size try to remove some
	if (m_PlayerCache.size() > MAX_CACHEDPLAYER)
	{
		// if we all of a sudden get a huge cache, try reducing faster
		int factor = m_PlayerCache.size() / MAX_CACHEDPLAYER + CACHEPLAYER_AGING_FACTOR;
		int ageTime = CACHEPLAYER_AGE / factor;
		ClearUpPlayerCache(ageTime);
	}
	//	Add player to table
	ACSWrapper cspt(&m_csPlayerTab);
...
void CECPlayerMan::ElsePlayerLeave(int cid, bool bExit, bool bUpdateMMArray/* true */)
{
	...
	/*	if (pHost && pHost->IsTeamMember(cid))
		...
		// IF the player was fully loaded, lets cache it
		if (pPlayer->IsAllResReady()/* && !pPlayer->IsShapeModelChanged() */)
		{
		//	Push player data into cache
		CECEPCacheData* pCacheData = pPlayer->CreateCacheData();
			pCacheData->m_pPlayer = pPlayer;
		if (pCacheData)
			PushPlayerToCache(pCacheData);

			// Comes from ReleaseElsePlayer, we want this but not actual delete of player
	...
	// // Do not release else player here
	// ReleaseElsePlayer(pPlayer);
}
...
CECElsePlayer* CECPlayerMan::CreateElsePlayer(const S2C::info_player_1& Info, int iAppearFlag)
{
	...
	if (pCacheData)
	{
		pPlayer = pCacheData->m_pPlayer;
		pPlayer->SetBornStamp(g_pGame->GetGameRun()->GetWorld()->GetBornStamp());
		
		// get the cached player shape
		int iShape = pPlayer->GetShapeID();
		bool wasOnPet = pPlayer->IsRidingOnPet();
		pPlayer->Init(Info, iAppearFlag, true);

		// if player shape is now different, we need to transform 
		if (iShape != pPlayer->GetShapeID())
			pPlayer->TransformShape(pPlayer->GetShapeID(), true);

		// Update fly and mount state
		pPlayer->ShowWing(pPlayer->IsFlying());

		if (pPlayer->IsRidingOnPet())
		{
			CECPlayer::RIDINGPET petInfo = pPlayer->GetRidingPetInfo();
			pPlayer->RideOnPet(petInfo.id, petInfo.color);
		}
		else if (wasOnPet)
		{
			pPlayer->GetOffPet();
		...
		/* if player was fully loaded, do we need to do this or something else? 
	...
	{
		pPlayer = new CECElsePlayer(this);
		if (!pPlayer)
		{
...
//	Clear up player data cache
void CECPlayerMan::ClearUpPlayerCache(int iAgeTime /* 300 */)
{
	//if (m_PlayerCache.size() < MAX_CACHEDPLAYER)
	//	return;
	...
	{
		CECEPCacheData* pData = *it.value();
		if (pData->m_dwCacheTime + iAgeTime < dwCurTime)
		{
...
void CECPlayerMan::UpdatePlayerInRanges(DWORD dwDeltaTime)
{
	...
	while(it != m_PlayerTab.end())
	{
		...
		if (fDistToHost <= 60.0f || pPlayer->GetCharacterID() == idHoverObject || pPlayer->GetCharacterID() == idSelected)
			if(!pHost->GetCameraOnFocus())
	...
	bool bClient)
{
	...
	pInfo->bClientModel = bClient;    // ���ͻ���ģ��
...
void CECPlayerMan::TabSelectCandidates(int idCurSel, APtrArray<CECElsePlayer*>& aCands)
{
	...
	if (!pHost->IsSkeletonReady()){
		//	ֻ�� IsSkeletonReady() Ϊ true ʱ GetDistToHost() ����Ч
		return;
	}
...
{
	//	���ݵ�ǰ�Ż����ã����� NPC �ϵ� Extend States ��Ч��ʾ
	PlayerTable::iterator it = m_PlayerTab.begin();
	for (; it != m_PlayerTab.end(); ++it)
	{
		CECElsePlayer* pPlayer = *it.value();
