...
void CDlgCountryMap::StartMove()
{
	...
	m_moveState = MS_WAITING_MOVE;	//	���õȴ��ƶ�״̬
	m_domainNext = 0;				//	����ƶ�Ŀ�ꡢ�ƶ���ʱ
...
{
	//	��ȡ��ǰ�����ڵ㵽������Ŀ�껨��ʱ��
	//	MS_WAITING_REDIRECT ״̬�£�m_route ����ض������·������֮ǰ·��
...
void CDlgCountryMap::UpdateAdditionalInfo()
{
	...
	for (int i(0); i < DOMAIN2_INFO::COUNTRY_COUNT; ++ i)
	{
		...
		int addcount = floor(players[i] / 3);

		strName.Format("Lbl_People%d", i+1);
		if (pObj = GetDlgItem(strName))
			pObj->SetText(strText.Format(_AL("%d"), players[i] + addcount));
...
{	
	if (GetGameSession()->IsConnected()) //	����Ͽ����Ӻ��޷��رտͻ���
	{
		if (!IsStopped())
		{
...
{	
	...
	enum{
		REASON_SYNC_MOVE = 0,		//	��һ���ƶ�����ɹ����
		REASON_SYNC_PREV_STEP,		//	�쳣��������һ������
		REASON_SYNC_CAPITAL,		//	�쳣�������׶�
		REASON_SYNC_CLI_REQUEST,	//	�ͻ������󷵻�
		REASON_SYNC_FINISH,			//	ս������������ͻ����ƶ�״̬
	};
