...
BOOL SafeGetDiskFreeSpace(LPCSTR pszDrive) {
	...
	if (freeByte < 50 * 1024 * 1024) {
		MessageBoxA(NULL, "Ӳ�̿ռ䲻�㣬�޷������������!", "����", MB_OK | MB_ICONSTOP);
		return FALSE;
	}
...
void CECTerrainPreprocessor::SaveFile(const char* szMapName) {
	...
	if ((int)subTernInfo.size() != fileHeader.iNumSubTrn) {
		a_LogOutput(1, "CECTerrainPreprocessor::SaveFile��%s �ӵ�ͼ��������", szMapName);
		return;
	}
...
int CECRandomMapProcess::GetNewSubTernIndexByOldIndex(int oldIndex) { // ԭ���ĵڼ��飬���ڱ�ɵڼ���
...
int CECRandomMapProcess::GetOldSubTernIndexByNewIndex(int oldIndex) { // ���ڵĵڼ��飬��ԭ���ĵڼ���
...
void CECRandomMapProcess::ProcessRepeatSubTern() {
	...
	abase::vector<int> oldSub; // ԭ���У������û�е����.
	...
	abase::vector<RepeatSubTrn> newSub; // ������ظ�����ţ��ӵڶ��γ��ֿ�ʼ��¼��Ϣ
	for (int i = 1; i < c; i++) {
		for (j = 0; j < i; j++) {
	...
	for (int i = 0; i < (int)newSub.size(); i++) {
...
void CECRandomMapProcess::GenerateMiniMap() {
	...
	// Testing
	//AString szDir;
	//szDir.Format("temp\\test\\minimap\\");
	//glb_MakeDir(szDir, szDir.GetLength());
			...
			for (int r = baseRow; r < baseRow + imgHCount; r++) {
				for (int c = baseCol; c < baseCol + imgWCount; c++) {
					...
					if (bval)
					{
						...
						// Testing
						//AString szFileN;
						//szFileN.Format("temp\\test\\minimap\\pre_%d.dds", index);
						//pA3DSurface->SaveToFile(szFileN);
					}
					else
					{
						a_LogOutput(1, "CECRandomMapProcess::GenerateMiniMap could not load image %s", fileName);
					}
				}
			}
...
void CECRandomMapProcess::GenerateImage(abase::vector<A3DSurface*>& srcImgVec, const AString& path, const AString& fileName) {
	...
	bool created = pDestSurface->Create(g_pGame->GetA3DDevice(), W, H, A3DFMT_X8R8G8B8);
	if (!created)
	{
		a_LogOutput(1, "CECRandomMapProcess::GenerateImage could not create surface %d, %d for file: %s", W, H, fileName);
	...
	for (i = 0; i < nRows; i++) {
		for (j = 0; j < nCols; j++) {
			...
			if (nIndex < (int)srcImgVec.size()) {
				...
				a_rc[0].left = W0 * j;
				a_rc[0].top = H0 * i;
				a_rc[0].right = W0 * j + W0;
				a_rc[0].bottom = H0 * i + H0;

				D3DXLoadSurfaceFromSurface(pDestSurface->GetDXSurface(), NULL, a_rc, srcImgVec[nIndex]->GetDXSurface(), NULL, NULL, D3DX_FILTER_NONE, 0);
			}
		}
	}
	...
	if (nFormat == A3DFMT_A8R8G8B8 || nFormat == A3DFMT_R8G8B8) {
		...
		pA3DSurface->Create(g_pGame->GetA3DDevice(), W, H, A3DFMT_X8R8G8B8);
		...
		A3DRELEASE(pA3DSurface)
			...
			//a_LogOutput(1, "CECRandomMapProcess::GenerateImage save to file new surface %s", szFile);
	}
	else
	{
		//a_LogOutput(1, "CECRandomMapProcess::GenerateImage save to file %s", szFile);
...
void CECRandomMapFogMask::UpdateByPos(const A3DVECTOR3& pos) {
	...
	int c2 = min((int)FOGMASK_RESOLUTION, x + FOGRADIUS);
	int r1 = max(0, y - FOGRADIUS);
	int r2 = min((int)FOGMASK_RESOLUTION, y + FOGRADIUS);
	for (int r = r1; r < r2; r++) {
