...
const char* szAction = "վ��_ͨ��";
const char* szCampIcon[] = {"2013\\ս��\\��֮ʥ����.dds","2013\\ս��\\��֮ʥ����.dds"};
//////////////////////////////////////////////////////////////////////////
...
void CChariot::OnChangeUI(CECGameUIMan* pUIMan,bool bBackup)
{
	...
	else
	{
		...
		if (pDlgRank)
		{
			...
			if (pRank)
			{
				pRank->OnCommandRankList(NULL) ; // �˳�ս��ʱ����һ��
			}
		}
	}
}
...
void CDlgChariotEnter::OnTick()
{
	...
	bool cond1 = (GetHostPlayer()->GetBasicProps().iLevel >= level && GetHostPlayer()->GetBasicProps().iLevel2 >= level2) || GetHostPlayer()->GetReincarnationCount() >=reincarnt; // ���� ������ħ
	bool cond2 = GetHostPlayer()->GetCoolTime(GP_CT_TRICKBATTLE_APPLY) <= 0; // ����ȴ
...
void CDlgChariotInfo::SelectChariot(int tid)
{
	...
	int skill_cnt = min((int)(sizeof(pChariot->skill)/sizeof(pChariot->skill[0])),(int)m_pImg_Skill.size());
...
void CDlgChariotRankList::OnTankBattleNotify(Protocol* pProtocol)
{
	...
	{
	case PROTOCOL_TANKBATTLEPLAYERAPPLY_RE: // npc �����ս�� ����
		{
		...
		break;
	case PROTOCOL_TANKBATTLEPLAYERGETRANK_RE: // ս������
		{
	...
	for(unsigned int i=0;i<m_rankList.size();i++)
	{
		...
		if(flag != NAME_DONE && id!=0)
		{
			...
			{	
				...
				m_pList->SetItemData(i,NAME_DONE,1); // �ѵõ�����
			}	
...
void CDlgChariotAmount::OnShowDialog()
{
	...
	for (int i=0;i<m_pImg_ChariotDefVec.size();i++)
	{
	...
	for (int i=0;i<m_pImg_ChariotAtkVec.size();i++)
	{
		CHARIOT_CONFIG* pChariotConfig = pChariot->GetChariot(i,0);
		if(!pChariotConfig) return;
...
{
	unsigned int i=0;
	for (int i=0;i<m_pProg_AmountAtkVec.size();i++)
	{
		m_pProg_AmountAtkVec[i]->SetProgress(0);
	}
	for (int i=0;i<m_pProg_AmountDefVec.size();i++)
	{
		m_pProg_AmountDefVec[i]->SetProgress(0);
	}
	for (int i=0;i<m_pLbl_NumberAtkVec.size();i++)
	{
		m_pLbl_NumberAtkVec[i]->SetText(_AL("0"));
	}
	for (int i=0;i<m_pLbl_NumberDefVec.size();i++)
	{
...
void CDlgChariotAmount::SetInfo(int atk,S2C::cmd_player_query_chariots::chariot* pAtk,int def,S2C::cmd_player_query_chariots::chariot* pDef)
{
	...
	//������ʾ
	ACString str;
	for (int i=0;i<(int)chariotAmountAtk.size();i++)
	{
	...
	for (int i=0;i<(int)chariotAmountDef.size();i++)
	{
