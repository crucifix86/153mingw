...
bool CECShopItemCategory::MatchItem(const GSHOP_ITEM &item)const{
	return MatchMainType(item.main_type)					//	��������Чʱ�ż��
		&& (m_subType < 0 || item.sub_type == m_subType);	//	�η���������С�������Ը���ֵ
}
...
bool CECShopItemOwnerNPC::HasOwnerNPC(const GSHOP_ITEM &item){
	return item.owner_npcs[0] != 0;			//	�����༭����֤������У���һ���Ƕ�����ǰ��
}
...
/*
* This function is related to Limited Items (can be purchased X times a week, or a day, month, etc).
* It set the amount of purchases given, for an item, in a given limitType (day, week, month..)
* 
*/
void CECShopBase::SetLimitedItem(int limitType, int itemId, int amountPurchased)
{
	if (limitType > CASH_VIP_SHOPPING_LIMIT_NONE && limitType < CASH_VIP_SHOPPING_LIMIT_COUNT)
	{
		m_limitedItems[limitType][itemId] = amountPurchased;

		for (std::map<int, int>::const_iterator it = m_limitedItems[limitType].begin(); it != m_limitedItems[limitType].end(); ++it)
		{
			a_LogOutput(1, "item %d, amount %d", it->first, it->second);
		}
	}
}
...
/*
* This function returns the amount of purchases for a given item, in a given limitType (also called limit_mode)
* Used to calculate several things related to the topic in QShop UI.
* 
* @return int Amount of purchases
*/
int CECShopBase::GetItemPurchasesAmount(int limitType, int itemId)
{
	for (std::map<int, int>::const_iterator it = m_limitedItems[limitType].begin(); it != m_limitedItems[limitType].end(); ++it)
	{
		a_LogOutput(1, "item %d, amount %d", it->first, it->second);
	...
	if (m_limitedItems[limitType][itemId])
		purchasesAmount = m_limitedItems[limitType][itemId];
	return purchasesAmount;
}

/*
* This function clear dataset of Limited Items for this account. Required when player logout, and used in HostPlayer when it gets released
*/
void CECShopBase::Clear()
{
	// Clear list of limited Items for this account.
	CECShopBase::m_limitedItems.clear();
}

//	CECShopArrayItems
CECShopArrayItems::~CECShopArrayItems() {}
...
//	CECVipShop
CECVipShop& CECVipShop::Instance()
{
	static CECVipShop s_instance;
	return s_instance;
}

CECVipShop::CECVipShop()
	: CECShopArrayItems(globaldata_getgshopitems3())
{
	if (IsFashionShopEnabled()) {
		m_fashionShopCategory.InitFromQShopConfig(CECQShopConfig::CID_QSHOP_FASHION);
	}
...
unsigned int CECVipShop::GetLocalTimeStamp()const {
	return globaldata_getgshop_timestamp3();
}

unsigned int CECVipShop::GetServerTimeStamp()const {
	return g_pGame->GetGameRun()->GetGShopTimeStamp3();
}

bool CECVipShop::GetFromServer(int beginIndex, int endIndex) {
	bool result = false;
	if (!g_pGame->GetGameRun()->GetHostPlayer()->GetCoolTime(GP_CT_GET_CASH_VIP_MALL_PRICE)) {
		g_pGame->GetGameSession()->c2s_CmdGetCashVipMallItemPrice(0, 0);
		result = true;
	}
	else if (beginIndex != 0 || endIndex != 0) {
		g_pGame->GetGameSession()->c2s_CmdGetCashVipMallItemPrice(beginIndex, endIndex);
		result = true;
	}
	return result;
}

bool CECVipShop::UpdateFromServer(const S2C::cmd_cash_vip_mall_item_price* pCmd) {
	GShopItems items = *m_pItems;
	if (!ApplyChangesFromServer(&items, pCmd)) {
		return false;
	}
...
bool CECVipShop::ApplyChangesFromServer(GShopItemsPtr pItems, const S2C::cmd_cash_vip_mall_item_price* pCmd)
{
	int i(0);
	for (i = pCmd->start_index; i < pCmd->end_index;i++) {
		GSHOP_ITEM& data = (*pItems)[i];
	...
	for (i = 0;i < pCmd->count;i++) {
		const S2C::cmd_cash_vip_mall_item_price::good_info& tempList = pCmd->list[i];
		GSHOP_ITEM& data = (*pItems)[tempList.good_index];
		if (data.id == (unsigned int)tempList.good_id) {
			data.buy[tempList.good_slot].price = tempList.good_price;
			data.buy[tempList.good_slot].status = tempList.good_status;
...
int CECVipShop::GetCash()const
{
	extern CECGame* g_pGame;
	return g_pGame->GetGameRun()->GetHostPlayer()->GetVipScore();
}

bool CECVipShop::Buy(int itemIndex, int buyIndex)const
{
	bool bOK(false);

	if (ReadyToBuy(itemIndex, buyIndex)) {
		const GSHOP_ITEM* pItem = GetItem(itemIndex);
		C2S::cmd_cash_vip_mall_shopping::goods item;
		item.goods_index = itemIndex;
		item.goods_id = pItem->id;
		item.goods_slot = buyIndex;
		if (!CECShopItemOwnerNPC::HasOwnerNPC(*pItem)) {
			g_pGame->GetGameSession()->c2s_SendCmdCashVipMallShopping(1, &item);
		}
		else {
			g_pGame->GetGameSession()->c2s_CmdNPCSevCashVipMallShopping(1, &item);
		}
		CECQShopConfig::Instance().OnItemBuyed(item.goods_id);
		CECShoppingItemsMover::Instance().OnItemBuyed(this, itemIndex, buyIndex);
		bOK = true;
...
bool CECVipShop::IsFashionShopEnabled()const {
	return CECUIConfig::Instance().GetGameUI().bEnableQShopFashionShop;
}

bool CECVipShop::IsFashionShopFlashSaleEnabled()const {
	return CECUIConfig::Instance().GetGameUI().bEnableQShopFashionShopFlashSale;
}

const ACString& CECVipShop::GetFashionShopFlashSaleTitle()const {
	return CECUIConfig::Instance().GetGameUI().strQShopFashionShopFlashSaleTitle;
}

const CECShopItemCategory& CECVipShop::GetFashionShopCategory()const {
	return m_fashionShopCategory;
}