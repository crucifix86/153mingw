...
bool CECSCCommand::Execute()
{
	...
	case CMD_PICKUPALL:			pHost->CmdPickupAll();								break;
	case CMD_QUICKTELEPORT:			pHost->CmdQuickTeleport();								break;
	default:
		return false;
	}
...
const char* CECSCCommand::GetIconFile()
{
	...
	case CMD_PICKUPALL:			szIconFile = res_IconFile(RES_ICON_CMD_PICKUPALL);			break;
	case CMD_QUICKTELEPORT:			szIconFile = res_IconFile(RES_ICON_CMD_QUICKTELEPORT);	break;
...
const wchar_t* CECSCCommand::GetDesc()
{
	...
	case CMD_PICKUPALL:			szDesc = pDescTab->GetWideString(CMDDESC_PICKUPALL);			break;
	case CMDDESC_QUICKTELEPORT:			szDesc = pDescTab->GetWideString(CMDDESC_QUICKTELEPORT);break;
...
int CECSCCommand::GetCoolTime(int* piMax/* NULL */)
{
	...
	case CMD_QUICKTELEPORT:

		iTime = pHost->GetCoolTime(GP_CT_FIX_POSITION_TRANSMIT, piMax);
		break;
	default:
...
///////////////////////////////////////////////////////////////////////////
//	
//	Implement CECSCAutoEquip
//	
///////////////////////////////////////////////////////////////////////////

CECSCAutoEquip::CECSCAutoEquip()
{
	m_iSCType = SCT_AUTOEQUIP;

	m_iEquipIdx = -1;
	strcpy(m_IconFile, "Iconset\\ac1.dds");
}

CECSCAutoEquip::CECSCAutoEquip(const CECSCAutoEquip& src)
{
	m_iEquipIdx = src.m_iEquipIdx;
	strcpy(m_IconFile, src.m_IconFile);
}

//	Initialize object
bool CECSCAutoEquip::Init(int iEquipIdx)
{
	m_iEquipIdx = iEquipIdx;

	char temp[10];
	itoa(iEquipIdx + 1, temp, 10);
	sprintf(m_IconFile, "Iconset\\ac%s.dds", temp);

	return true;
}

//	Clone shortcut
CECShortcut* CECSCAutoEquip::Clone()
{
	return new CECSCAutoEquip(*this);
}

//	Execute shortcut
bool CECSCAutoEquip::Execute()
{
	CECHostPlayer* pHost = g_pGame->GetGameRun()->GetHostPlayer();
	pHost->EquipEquipmentsBySuitID(m_iEquipIdx);
	return true;
}

//	Get icon file
const char* CECSCAutoEquip::GetIconFile()
{
	return m_IconFile;
}

//	Get shortcut description text
const wchar_t* CECSCAutoEquip::GetDesc()
{
	return _AL("");
}
