...
// 		else
		...
		m_pA3DTerrain->DoBlocksCull(pViewport->GetA3DViewport());
...
// ctor initializes to 0...not needed here
//	g_pGame->GetA3DEngine()->GetA3DLitModelRender()->SetAlphaMeshesArray(0);
	...
	{
		PROFILE_SCOPE("SkinModelNoAlpha");
		//	Render non alpha skin models
		psr1->Render(pA3DViewport, false);
		psr1->ResetRenderInfo(false);
	}
	{
		PROFILE_SCOPE("GrassNA");
...
bool CECWorld::RenderForReflected(A3DViewport* pViewport)
{
...
#ifdef REFLECT_SKINMODEL  // Do we really need skin models reflected?
	//	Use a temporary skin render so that we don't break world's main rendering
	A3DSkinRender* psr = g_pGame->GetSkinRender2();
	A3DSkinRender* poldsr = (A3DSkinRender*)g_pGame->GetA3DEngine()->GetA3DSkinMan()->GetCurSkinRender();
	...
	for (int i=0; i < NUM_MANAGER; i++)
	{
		if (m_aManagers[i])
		{
			...
			//else if (i != MAN_ORNAMENT )
			else
			{
				//char scopeStr[20];
				//sprintf(scopeStr, "Manager_%d", i);
				//PROFILE_SCOPE(&scopeStr[0]);
				m_aManagers[i]->RenderForReflect(&ecViewport);
			}
		}
	}
