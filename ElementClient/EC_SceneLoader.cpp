...
extern cxxpool::thread_pool* g_pLoadThreadPool;
...
public:
	void ProcessOneECM(CECPlayerMan* pPlayerMan, CECNPCMan* pNPCMan, CECOrnamentMan* pOnmtMan, CECMatterMan* pMatterMan, MTL_ECMODEL& Info, DWORD& dwSleep);
...
// Increase this to 80 per loop, 4 is a bit small
static const int _load_per_time = 4 * 3;

#include "EC_GameUIMan.h"
#include "EC_UIManager.h"
void ECModelLoaderModule::ProcessOneECM(CECPlayerMan* pPlayerMan, CECNPCMan* pNPCMan, CECOrnamentMan* pOnmtMan, CECMatterMan* pMatterMan, MTL_ECMODEL& Info, DWORD& dwSleep)
{
	DWORD dwTick;
	switch (Info.iType)
	{
	case MTL_ECM_PLAYER:
		...
		// Why pausing for upto 1 second between loading 4 models
...
inline void ECModelLoaderModule::LoadAndReleaseECModels(bool bInLoaderThread)
{
	...
	// Moved it below? See if it helps
	//::LeaveCriticalSection(&m_csDelECModel);
	...
	if (size > 0)
	{
		...
		for (int i = 0, max = size; i < max; i++)
		{
			...
			ProcessOneECM(pPlayerMan, pNPCMan, pOnmtMan, pMatterMan, Info, dwSleep);
