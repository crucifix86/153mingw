...
static const char* DEFAULT_IMG = "version01\\���ֱ���\\����.tga";
static const char* DEFAULT_GFX = "����\\���䷢��.gfx";
...
{
	CDlgExclusiveAward* pExclu = (CDlgExclusiveAward*)GetGameUIMan()->GetDialog("Win_ExclusiveAward");
	bool bCheck = pExclu->GetExclusiveAwardMode(); // true: ԭ������Ϊ�� false�� �ý���һֱ��ʾ��
	...
	if(m_RemainTime > 0)
	{
		...
		if(m_LastTime > 0)
		{
			unsigned long elapsed = (curTime > m_LastTime) ? (unsigned long)(curTime - m_LastTime) : 0UL;
			unsigned long eclipse = min(m_RemainTime, elapsed);
