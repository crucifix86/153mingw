...
inline bool LoadAwardMonstersBin(FILE* fp, AWARD_MONSTERS_SUMMONED& ms, unsigned long ulVersion)
{
	...
	for (int i=0; i < ms.m_ulMonsterNum; i++)
	{
		MONSTERS_SUMMONED& monster = ms.m_Monsters[i];
...
inline bool LoadAwardPQRankingBin(FILE* fp, AWARD_PQ_RANKING& pr, unsigned long ulVersion)
{
	...
	for (int i=0; i < pr.m_ulRankingAwardNum; i++)
	{
		RANKING_AWARD& rAward = pr.m_RankingAward[i];
...
inline bool LoadAwardData(FILE* fp, AWARD_DATA& ad, unsigned long ulVersion)
{
	...
	if (ulVersion >= 58)
	{
		...
		for (int i=0; i<ad.m_ulChangeKeyCnt; i++)
		{
			if (!ReadLine(fp, line)) return false;
	...
	if (ulVersion >= 112)
	{
		...
		for (int i=0; i<ad.m_ulHistoryChangeCnt; i++)
		{
			if (!ReadLine(fp, line)) return false;
	...
	{		
		...
		for (int i=0; i<ad.m_ulDisplayKeyCnt; i++)
		{
			if (!ReadLine(fp, line)) return false;
	...
	if (ulVersion >= 61)
	{
		...
		for (int i=0; i<ad.m_ulTaskCharCnt; i++)
		{
...
bool ATaskTemplFixedData::LoadFixedDataFromTextFile(FILE* fp, unsigned long ulVersion)
{
	...
	if (ulVersion >= 11)
	{
		...
		for (int i=0;i<m_ulDelvRegionCnt;i++)
		{
...
#endif
		for (int i=0;i<m_ulEnterRegionCnt;i++)
		{
...
#endif
		for (int i=0;i<m_ulLeaveRegionCnt;i++)
		{
...
#endif
		for (int i=0;i<m_ulReachSiteCnt;i++)
		{
...
#endif
		for (int i=0;i<m_ulLeaveSiteCnt;i++)
		{
	...
	if (ulVersion >= 62)
	{
		...
		for (int i=0; i<m_ulTaskCharCnt; i++)
		{
...
*/
	...
	for (int i=0;i<m_ulMonsterContribCnt; i++)
	{
...
ATaskTempl& ATaskTempl::operator= (const ATaskTempl& src)
{
	...
	*m_AwByItems_F = *src.m_AwByItems_F;
	...
	for (int i=0;i<MAX_CONTRIB_MONSTERS; i++)
