...
void OnTaskCheckStatus(TaskInterface* pTask)
{
	...
	for (i = 0; i < pLst->m_uTaskCount; i++)
	{
		...
		{
			if (pTempl->m_enumMethod == enumTMReachSite && pTempl->m_enumFinishType == enumTFTDirect) // ����ص��ж�
			{
			...
			if (pTempl->m_enumMethod == enumTMLeaveSite && pTempl->m_enumFinishType == enumTFTDirect) // �뿪�ص��ж�
			{
				if (ulCurTime - TaskInterface::GetFinishDlgShowTime() < FINISH_DLG_SHOWN_TIME)
					continue;
