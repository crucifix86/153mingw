...
enum
{
	...
	CDLGNPC_ADD_ACCESORY_SOCKET =  0xFFFFF56,
};
...
void CDlgNPC::ReleaseCamera() {
				...
				for(int i = 0; i < PlayerNear.GetSize(); i++ )
				{
					PlayerNear[i]->SetVisible(true);
				//	PlayerNear[i]->SetRenderNameFlag(true);
				}
				CECNPCMan *pNPCMan = GetWorld()->GetNPCMan();
				CECNPC *pNPC = pNPCMan->GetNPC(GetHostPlayer()->GetSelectedTarget());
				if(pNPC) {
					pNPC->SetHighligh(false);
				}
}
...
void CDlgNPC::PopupNPCDialog(const NPC_ESSENCE *pEssence)
{
	...
	static const int ForbiddenNPCList[] = {0};
	vector<int> ForbiddenNPCListVector (ForbiddenNPCList, ForbiddenNPCList + sizeof(ForbiddenNPCList) / sizeof(ForbiddenNPCList[0]) );


	bool isForbiddenCamera = std::find(ForbiddenNPCListVector.begin(), ForbiddenNPCListVector.end(), pEssence->id) != ForbiddenNPCListVector.end();

	
		   // pGameRun->PostMessage(MSG_HST_FIXCAMERA, MAN_PLAYER, 0);

			CECNPCMan *pNPCMan = GetWorld()->GetNPCMan();
			CECNPC *pNPC = pNPCMan->GetNPC(GetHostPlayer()->GetSelectedTarget());	

			if(!isForbiddenCamera){
				if( pNPC )
				{
					
					A3DMATRIX4 matNPC = TransformMatrix(pNPC->GetDir(), A3DVECTOR3(0.0f, 1.0f, 0.0f), pNPC->GetPos());
					A3DVECTOR3 m_vPosDaltaM = A3DVECTOR3(0.0f, 0.0f, 1.0f);
					A3DVECTOR3 vecPlayerPos = A3DVECTOR3(0.0f, 0.5f, 0.0f) * matNPC;
					A3DVECTOR3 vecCamPos;

					const A3DAABB& aabb = pNPC->GetPickAABB();
					A3DVECTOR3 vPos = pNPC->GetPos();
					vPos.y = aabb.Center.y + aabb.Extents.y * 1.15f;


					vecCamPos = (A3DVECTOR3(0.0f, 1.2f+ aabb.Extents.y/2, 15.0f) + m_vPosDaltaM)* matNPC;
					
					A3DVECTOR3 vecCamToPlayer = a3d_VectorMatrix3x3(A3DVECTOR3(0.0f, 0.0f, -1.0f), matNPC);

					A3DCamera * pCamera = g_pGame->GetViewport()->GetA3DCamera();
					pCamera->SetPos(vecCamPos,true);
					pCamera->SetDirAndUp(vecCamToPlayer, A3DVECTOR3(0.0f, 1.0f, 0.0f),true);
					pCamera->SetFOVSet(DEG2RAD(13.0f + aabb.Extents.y * 7.0f));
					pCamera->SetFOV(DEG2RAD(13.0f + aabb.Extents.y * 7.0f));

					CECHostPlayer * pPlayer = GetHostPlayer();
					pPlayer->SetVisible(false);
					pPlayer->SetRenderNameFlag(false);
					pPlayer->SetCameraOnFocus(true);
					pNPC->SetHighligh(true);
					CECWorld *pWorld = GetWorld();
					CECPlayerMan *pPlayerMan = pWorld->GetPlayerMan();
					const APtrArray<CECElsePlayer *> &PlayerNear = pPlayerMan->GetPlayersInMiniMap();
					for(int i = 0; i < PlayerNear.GetSize(); i++ )
					{
						PlayerNear[i]->SetVisible(false);
						PlayerNear[i]->SetRenderNameFlag(false);
					}
				}
			}
...
// 			m_pLst_Main->SetItemData(m_pLst_Main->GetCount() - 1, CGLDNPC_FORTRESS_WARLIST);
	...
	if (pEssence->combined_services2 & 0x00080000) {
		m_pLst_Main->AddString(strText + GetStringFromTable(11503));
		m_pLst_Main->SetItemData(m_pLst_Main->GetCount() - 1, CDLGNPC_ADD_ACCESORY_SOCKET);
	}
...
bool CDlgNPC::PopupCorrespondingServiceDialog(
	int idFunction, int iService, void *pData, NPC_ESSENCE *pNPCESSENCE)
{
	NPC_ESSENCE *pCurNPCEssence = NULL;
	PAUIDIALOG pShow1 = NULL, pShow2 = NULL;
	if(pNPCESSENCE == NULL)
		pCurNPCEssence = GetGameUIMan()->m_pCurNPCEssence;
	else
		pCurNPCEssence = pNPCESSENCE;
...
void CDlgNPC::SelectListItem(int index)
{
	...
	else if( GetData() == NPC_DIALOG_ESSENCE )
	{
		...
		if( pEssence )
		{
			...
			else if (iService == CDLGNPC_ADD_ACCESORY_SOCKET && (pEssence->combined_services2 & 0x00080000))
			{
				Show(false);
				ReleaseCamera();
				GetGameUIMan()->m_pDlgEquipAccessorySlot->Show(true);
				GetGameUIMan()->m_pDlgInventory->Show(true);
				return;
			}
		}
