...
CECIvtrEquip::CECIvtrEquip(const CECIvtrEquip& s) : CECIvtrItem(s)
{
	...
	for (int i=0; i < s.m_aHoles.GetSize(); i++)
		m_aHoles[i] = s.m_aHoles[i];

	m_aProps.SetSize(s.m_aProps.GetSize(), 4);
	for (int i=0; i < s.m_aProps.GetSize(); i++)
...
//	Get Addons code number
int CECIvtrEquip::GetAddonCode()
{
	int iCount = 0;

	for (int i = 0; i < GetPropertyNum(); i++)
	{
		const PROPERTY& py = m_aProps[i];
			if (py.iType != 0)
				iCount += py.iType;
	}
...
#define ADD_RANGE_VALUE_DESC_STR_NORMAL(id_string)					ADD_RANGE_VALUE_DESC_STR((id_string), _AL("%+d"), _AL("~%d"), p0, p1, p0)
	...
	if (!IsSharpenerProperty(byPropType)){
		...
		case 163: //Physical Defense(Astrolabe)
			AddDescText(color, false, pDescTab->GetWideString(ITEMDESC_PDEF));
			AddDescText(color, true, _AL(" %+d"), p0);
			break;
		...
		case 172: //Hit(Astrolabe)
			AddDescText(color, false, pDescTab->GetWideString(ITEMDESC_ACCURACY2));
			AddDescText(color, true, _AL(" %+d"), p0);
			break;

		case 173: //Dodge Degree(Astrolabe)
			AddDescText(color, false, pDescTab->GetWideString(ITEMDESC_EVASION));
			AddDescText(color, true, _AL(" %+d"), p0);
			break;
		...
		case 175: //Ignore physical defense level(astrolabe)
			AddDescText(color, false, pDescTab->GetWideString(ITEMDESC_PPEN));
			AddDescText(color, true, _AL(" %+d"), p0);
			break;

		case 176: //Ignore Magic Defense Level(Astrolabe)
			AddDescText(color, false, pDescTab->GetWideString(ITEMDESC_MPEN));
			AddDescText(color, true, _AL(" %+d"), p0);
			break;
...
void CECIvtrEquip::BuildTesseraDesc()
{
	...
	for (int i=0; i < m_aHoles.GetSize(); i++)
	{
		...
		if (pItem->GetClassID() == CECIvtrItem::ICID_STONE)
		{
			...
			else if ( m_iCID == ICID_DECORATION )
				szText = ((CECIvtrStone*)pItem)->GetDBEssence()->decoration_desc;
			else
