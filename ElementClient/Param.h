...
// Forward declarations of helper functions (used in CParamImpBase)
template<typename DataType> class CParamImp;
template<typename DataType> DataType GetParamTrueValue(const CParam *pParam);
template<typename DataType> void SetParamTrueValue(CParam *pParam, DataType val);
...
public:
	CParamImpBase(
		ACString strName,
		DataType valInit,
		int nDisplayType = CParam::TYPE_NORMAL,
