...
//	Search thourhg all products
// Class in charge of Searching items in a given shop.
class GShopSearcher
{
	CDlgQShop	*_pDlgShop;
	CECShopBase *_pShop;
...
public:
	GShopSearcher(CDlgQShop *pDlgShop, CECShopBase * pShopBase, const ACString &searchPattern)
		: _pDlgShop(pDlgShop)
		, _pShop(pShopBase)
		, _id(0)					//	默认不能切换使用ID搜索
...
//	Order based on product "status"
// Class in charge of ordering products based on a criteria
class GShopCollector
{
	CDlgQShop::GShopItemsPtr _pItems;	//	Products list

	typedef CDlgQShop::QShopItemIndexArray QShopItemIndexArray;

	enum {CATEGORY_COUNT = 5};
	QShopItemIndexArray _category[CATEGORY_COUNT];	//	Product categories, 13 = Flash sale, 3 recommended, 2 = promotion, 1 = new, and others.
...
AUI_ON_COMMAND("Btn_VipHelp",	OnCommand_VipHelp)
...
bool CDlgQShop::OnInitDialog()
{
	...
	// Vip
	m_pVipHelp = (PAUISTILLIMAGEBUTTON)GetDlgItem("Btn_VipHelp");
	m_pTxt_VipPoints = (PAUILABEL)GetDlgItem("Txt_VipPoints");
	m_pPro_VipPro = (PAUIPROGRESS)GetDlgItem("Pro_VipPro");
	m_pPro_VipFull = (PAUIPROGRESS)GetDlgItem("Pro_VipFull");
	m_pImg_Attention = (PAUIIMAGEPICTURE)GetDlgItem("Img_Attention");
...
{
	// To keep a unified window, get player level when qshop is showing, so we can filter by level
	m_playerLevelOnShow = GetHostPlayer()->GetMaxLevelSofar();

	// Get player VIP info
	m_playerVipLevel = 0;
	m_playerVipScore = 0;
	if (GetHostPlayer()->IsVipInitialized()) {
		m_playerVipLevel = GetHostPlayer()->GetVipLevel();
		m_playerVipScore = GetHostPlayer()->GetVipScore();
	}
...
void CDlgQShop::OnCommand_CANCEL(const char * szCommand){
	...
	if (GetShopDataConst().HasOwnerNPC()){
		CECUIHelper::EndNPCGoldShopService();		//	When closing the shop, the NPC service needs to be closed too
	}
}
...
void CDlgQShop::OnTick()
{
	...
	if (IsQShop())
	{
		...
		// Vip level info
		ACString strText;
		m_pVipHelp->SetText(strText.Format(_AL("VIP %d"), m_playerVipLevel)); ///TODO: Is there a localized string somewhere that says "Star" or "VIP" ?

		// Vip score (points) information and progress bar
		ACString strText2;
		// Progress bar and information is different if player reached MAX Vip level or not.
		int scoreNeededNextLevel = CECVipConfig::Instance().GetVipScoreNeededByLevel(m_playerVipLevel + 1);
		if (!CECVipConfig::Instance().ReachedVipMaxLevel(m_playerVipLevel)) {
			m_pTxt_VipPoints->SetText(strText2.Format(_AL("%d/%d"), m_playerVipScore, scoreNeededNextLevel)); ///TODO: Localize string
			int remainingCashForNextLevel = 0;
			remainingCashForNextLevel = std::ceil(((float)scoreNeededNextLevel - (float)m_playerVipScore) / 3);
			m_pImg_Attention->SetHint(strText2.Format(GetStringFromTable(11752), remainingCashForNextLevel, (m_playerVipLevel + 1)));
			m_pPro_VipFull->Show(false);
			m_pPro_VipPro->SetProgress((float)m_playerVipScore / (float)scoreNeededNextLevel * 100);
		}
		else {
			m_pTxt_VipPoints->SetText(strText2.Format(_AL("%d"), m_playerVipScore)); ///TODO: Localize string
			m_pPro_VipPro->Show(false);
			m_pPro_VipFull->SetProgress((float)m_playerVipScore / (float)scoreNeededNextLevel * 100);
			m_pImg_Attention->SetHint(strText2.Format(GetStringFromTable(11753), CECVipConfig::Instance().GetVipScoreReducePerDayByLevel(m_playerVipLevel)));
		}
#endif
		...
		strText3.Format(GetGameUIMan()->GetStringFromTable(10014), fCashRate);

		if( GetHostPlayer()->IsCashMoneyOpen() )
		{
			m_pTxt_CashMoneyRate->SetText(strText3);
			m_pTxt_CashMoneyRate->SetHint(_AL(""));
		}
		else
		{
			m_pTxt_CashMoneyRate->SetText(GetGameUIMan()->GetStringFromTable(10017));
			m_pTxt_CashMoneyRate->SetHint(strText3);
		}
	}
...
void CDlgQShop::ShowMainType(int nCommandMainType) {
	...
	// Check if this is Fashion shop, if so, show Fashion window, and current shop show other categories.
...
{
	//	When scroll change position, update product screen

	if (!IsShow())
	{
		//	When dialog is not showing, leave update to OnTick
		//	Otherwise CDlgShopItem will show when CDlgQshop is hidden (when interface changes)
		return;
	}
	...
	if( m_nStartLine != m_pScl_Item->GetBarLevel() )
	{
		...
		int i;
		for (i = nStart; i < nEnd; i++) {// Populate items to show in the screen.
			int vipItemIndex = GetCashVipItemIndex(m_vecIdItem[i]);
			if (vipItemIndex > -1) {
				GetGameUIMan()->m_pDlgQShopDefault[w0++]->SetItem(i - nStart, vipItemIndex, &CECVipShop::Instance());
			}
			else {
				GetGameUIMan()->m_pDlgQShopDefault[w0++]->SetItem(i - nStart, m_vecIdItem[i], &GetShopData());
			}
			
		}
		for(i = w0; i < CECGAMEUIMAN_MAX_QSHOPITEMS; i++) // Clean rest of the list if there is items to show are less than CECGAMEUIMAN_MAX_QSHOPITEMS.
...
int CDlgQShop::GetVipLevel()
{
	return GetHostPlayer()->GetVipLevel();
}
int CDlgQShop::GetVipScore()
{
	return GetHostPlayer()->GetVipScore();
}
...
bool CDlgQShop::SearchByPattern(const ACString &strPattern, QShopItemIndexArray &result, int maxSearchCount)
{
	...
	GShopSearcher searcher(this, &this->GetShopData(), strPattern);
	GShopCollector collector(pItems);

	int matched = -1;

	//	First search using match by string
	...
	//	When it cant find by string, it will try to find by ItemID
...
{
	// Search item with itemIndex, change to main category and sub category accordingly and select the product
...
/*
* This function check if a given item belongs to the CashVip shop.
* 
*/
bool CDlgQShop::IsCashVipShopItem(GSHOP_ITEM* item) {
	return (IsQShop() && item->main_type == 5);
}

void CDlgQShop::OnCommand_VipHelp(const char* szCommand)
{
...
/*
* This function receives an itemIndex and return the itemIndex valid for CashVip shop
* since CashVip Shop items were placed at the end of the regular Cash shop items array.
* See globaldataman.cpp, while loading gshop2.data
* 
* We use itemIndex for qshop when in DlgQShop, and proper Index (specific for each shop) 
* in DlgQShopItem and DlgQShopBuy to handle items appriatelly when rendering them
* 
* @return -1 if this is not a CashVip shop, index if valid.
* 
*/
int CDlgQShop::GetCashVipItemIndex(int itemIndex) {

	if (!IsQShop()) return -1;
	// Is Vip Item
	int countItemsQshop = CECQShop::Instance().GetCount();
	int countItemsVipShop = CECVipShop::Instance().GetCount();
	int lastQshopItem = countItemsQshop - countItemsVipShop;
	if (itemIndex >= lastQshopItem) {
		return itemIndex - lastQshopItem;
	}
	return -1;
}

/*
* This function receives an index of a (previously checked) CashVip item
* and returns a valid index for the regular cash Shop.
* Remember: CashVip items were loaded at the end of the regular Cash items array in order
* to unify qshop and vipshop in a single UI interface.
*
* @return qshop valid itemIndex
*
*/
int CDlgQShop::GetItemIndexFromVipItemIndex(int vipItemIndex) {
	// vipItemIndex must be a VIP Item. Check that before calling this function
	int countItemsQshop = CECQShop::Instance().GetCount();
	int countItemsVipShop = CECVipShop::Instance().GetCount();
	int lastQshopItem = countItemsQshop - countItemsVipShop;

	return vipItemIndex + lastQshopItem;
