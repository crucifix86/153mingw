...
Mouse::Mouse()
{
	...
	int i;
	for( int i=0; i<sizeof(g_counts)/sizeof(g_counts[0]); i++)	g_counts[i] = 0;
	for( int i=0; i<sizeof(g_times)/sizeof(g_times[0]); i++)	g_times[i] = 0;
	for( int i=0; i<sizeof(g_rects)/sizeof(g_rects[0]); i++)	g_rects[i].left = g_rects[i].right = g_rects[i].top = g_rects[i].bottom = 0;
}
