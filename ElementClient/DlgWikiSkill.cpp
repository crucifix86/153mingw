...
void CDlgWikiSkill::OnEventLButtonDown_List(WPARAM wParam, LPARAM lParam, AUIObject *pObj)
{
	...
	if (pSkill)
	{
		...
		pTxt_LevelRequire1->SetText(level1 > 0 ? strTxt.Format(GetStringFromTable(1899), level1) : AWString(GetStringFromTable(786)));
		...
		pTxt_FirstLevel->SetText(firstLevel > 0 ? strTxt.Format(GetStringFromTable(1899), firstLevel) : AWString(GetStringFromTable(786)));
