...
#if 0  // BBS feature disabled for MinGW build
	if( 0 == stricmp(pDlg->GetName(), "Win_BBSDlg") )
		return m_BBSDlg.OnEvent(uMsg, wParam, lParam, pDlg, pObj);
#endif

#if 0  // Customize feature disabled for MinGW build
				...
				&& pObjOver && strstr(pObjOver->GetName(), "Item_") )
			{
				...
				if( pIvtrDst )
				{
						...
						pIvtrSrc->GetTemplateID() == pIvtrDst->GetTemplateID() )
					{
						...
						{
							pSession->c2s_CmdMoveIvtrToTrashBox(IVTRTYPE_TRASHBOX, iDst, iSrc,
								min(pIvtrSrc->GetCount(), pIvtrSrc->GetPileLimit() - pIvtrSrc->GetCount()));
						}
					}
					else
						pSession->c2s_CmdExgTrashBoxIvtrItem(IVTRTYPE_TRASHBOX, iDst, iSrc);
				}
				else
				{
					if( AUI_PRESS(VK_MENU) && pIvtrSrc->GetCount() > 1 && pIvtrSrc->GetPileLimit() > 1 )
			...
			if( iSrc != iDst )
			{
				if( pIvtrDst )
				{
						...
						pIvtrSrc->GetTemplateID() == pIvtrDst->GetTemplateID() )
					{
						...
						{
							pSession->c2s_CmdMoveTrashBoxItem(IVTRTYPE_TRASHBOX, iSrc, iDst,
								min(pIvtrSrc->GetCount(), pIvtrSrc->GetPileLimit() - pIvtrSrc->GetCount()));
						}
					}
					else
						pSession->c2s_CmdExgTrashBoxItem(IVTRTYPE_TRASHBOX, iSrc, iDst);
				}
				else
				{
					if( AUI_PRESS(VK_MENU) && pIvtrSrc->GetCount() > 1 && pIvtrSrc->GetPileLimit() > 1 )
			...
			m_pDlgInventory->GetShowItem() == CDlgInventory::INVENTORY_ITEM_NORMAL )
		{
			...
			if( pIvtrDst )
			{
					...
					pIvtrSrc->GetCount() > 1 &&	pIvtrSrc->GetPileLimit() > 1 )
				{
					...
					pSession->c2s_CmdExgTrashBoxIvtrItem(IVTRTYPE_TRASHBOX, iSrc, iDst);
			}
			else
			{
				if( AUI_PRESS(VK_MENU) && pIvtrSrc->GetCount() > 1 &&	pIvtrSrc->GetPileLimit() > 1 )
