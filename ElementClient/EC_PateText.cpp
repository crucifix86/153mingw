...
void CECPateText::Clear()
{
	...
	for (int i=0; i < m_aTextStrs.GetSize(); i++)
	...
	for (int i=0; i < m_aItems.GetSize(); i++)
	{
		...
		if (Item.iType == TYPE_TEXT)
		{
			...
			if (pFont)
			{
				...
				//fix white shadow
				unsigned int color = Item.clItem ? Item.clItem : dwCol;
				if (((color & 0xff) * 0.2 + ((color & (0xff << 8)) >> 8) * 0.5 + ((color & (0xff << 16)) >> 16) * 0.3) <= 100)
				{
					pFont->TextOut(x, y, *pstr, 0xFFFFFFFF, 0, z, true); //shadow
					pFont->TextOut(x, y, *pstr, color, 0, z, false);
				}
				else
				{
					pFont->TextOut(x, y, *pstr, color, 0, z, m_bBorder);
				}
