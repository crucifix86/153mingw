...
m_strName(_AL("NPC"))
{
	...
	m_iPassiveMove		= 0;
	m_bAboutToDie		= false;
	m_bRenderingTaskGFX = false;
	m_bRequiredForTask = false;
	m_bRenderName		= false;
	m_nFightTimeLeft	= 0;
	m_pCurSkill			= NULL;
...
CECNPC::~CECNPC()
{
	// ctor created it,so we should delete it too
	if ( m_pPateName != NULL )
	{
		delete m_pPateName;
		m_pPateName = NULL;
...
{
	if (IsUsingMasterModel()){
		ReleaseNPCModel(Ret);			//	���︴�ƽ�ɫģ��ʱ���������ڽ�ɫ��ʱ�����ڶ����¼�����NPCԭ��ģ�ͣ�
		return false;					//	���п�����ʱ���ֽ�ɫģ�Ͷ������˽�ɫģ�ͣ���ʱԭ���ؽ������ʹ��
	}
	BeforeChangeNPCModel();
...
bool CECNPC::TickBeforeAnimate(DWORD dwDeltaTime)
{
	...
	m_pNPCModelPolicy->TickBeforeAnimate(dwDeltaTime);

	return true;
}

bool CECNPC::TickAfterAnimate(DWORD dwDeltaTime)
{
	DWORD dwRealTime = g_pGame->GetRealTickTime();

	m_pNPCModelPolicy->TickAfterAnimate(dwDeltaTime);
	m_pNPCModelPolicy->UpdateModelAABB();
...
//	Tick routine
bool CECNPC::TickOnlyAnimation(DWORD dwDeltaTime)
{
	m_pNPCModelPolicy->TickAnimation(dwDeltaTime);
	return true;
}
bool CECNPC::Tick(DWORD dwDeltaTime)
{
#if 1
	TickBeforeAnimate(dwDeltaTime);
	TickOnlyAnimation(dwDeltaTime);
	TickAfterAnimate(dwDeltaTime);
#else
	CECObject::Tick(dwDeltaTime);
...
void CECNPC::ReleaseWork(int iWorkType)
{
	...
	case WORK_MOVE:
		{
			//	Ϊ��ֹ WORK_MOVE �� WORK_SPELL �� WORK ������� NPC λ�ô��󣬴�ʱ��鲢ֱ�ӽ� NPC ����Ŀ��λ��
...
void CECNPC::TickWork_PolicyAction(DWORD dwDeltaTime)
{
	...
	if (!m_pPolicyAction->IsInPolicy())
	{
		...
		//	���ڷ��������ܱ�֤ HostPlayer �ɶ��յ� OnMsgNPCStartPlayAction �� OnMsgNPCStopPlayAction
		//	�ͻ��������Ҫ�Լ���ʱ����������ǿ OnMsgNPCStartPlayAction �� OnMsgNPCStopPlayAction ��³����
		//
...
void CECNPC::PlayGfx(const char* szPath, const char* szHook)
{
	m_pNPCModelPolicy->PlayGfx(szPath, szHook);
}
void CECNPC::RemoveGfx(const char* szPath, const char* szHook)
{
	m_pNPCModelPolicy->RemoveGfx(szPath, szHook);
}
...
void NPCRenderDemonstration(const A3DRECT &rc, DWORD param1, DWORD param2, DWORD param3)
{
	...
	fMoveRatio *= 3.5f;		//	����ƶ���Χ
	a_ClampRoof(fMoveRatio, 4.7f);		//	���Զ�������Ƶ�������һ�ߣ�ʵ��ӦΪ deltaZ < dist => fMoveRatio < 4.7+1/ext
	float deltaZ = ext*fMoveRatio;
	float deltaY = height/dist*deltaZ;	//	��ԭ�����ƽ�/Զ������ﵽ�Ŵ�/��СЧ��
...
bool CECNPC::FilterFiltersWithParam(int state)
{
	return state == 287 ||
		state == 225 ||
		state == 164 ||
...
const ACHAR* CECNPC::GetNameToShow()const{
	return m_pNPCModelPolicy ? m_pNPCModelPolicy->GetNameToShow() : (const ACHAR*)m_strName;
}
