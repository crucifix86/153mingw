...
void CDlgHost::RefreshHostStats()
{
	...
	IconLayoutBig layout(m_vecImgState.size() / 2);
...
bool CDlgHost::RefreshGatherBar(bool bActive)
{
	...
	if( pHost->IsGathering() )
	{
		...
		int nMax = max(static_cast<int>(counter.GetPeriod()), 1);
...
bool CDlgHost::RefreshPowerBar(bool bOtherShown, bool bActive)
{
	...
	{
		// MIRAR E SACRIFICIO INSTANTANEO
		if (g_pGame->GetConfigs()->GetGameSettings().bAutoCharge) {
			pCurSkill->EndCharging();
...
bool CDlgHost::RefreshIncantBar(bool bOtherShown, bool bActive)
{
	...
	if(!bOtherShown)
	{
		...
		else if( pHost->IsOperatingPet() )
		{
			...
			nMax = max(static_cast<int>(pHost->GetPetOptTime().GetPeriod()), 1);
		...
		else if( pHost->IsSpellingMagic() && pCurSkill )
		{
			...
			nMax = max(static_cast<int>(counter.GetPeriod()), 1);
...
public:
	...
	virtual SIZE Update(const S2C::IconState& state, int x, int y, PAUIIMAGEPICTURE pIcon)
	{
		...
		ACString buffstr = g_pGame->GetBuffDesc(pTS->GetID());
		ACString finaltext;
		...
		if (curVal > 86400) {
			int days = curVal / 86400;
			int remainofday = curVal % 86400;

			int hour = remainofday / 3600;
			int remainofhour = remainofday % 3600;
			int minute = remainofhour / 60;

			pIcon->SetHint(finaltext.Format(_AL("%s \rTime remaining: %d days, %d hours and %d minutes."), buffstr, days, hour, minute));
		}
		else if (curVal > 3600) {
			int hour = curVal / 3600;
			int remainofhour = curVal % 3600;
			int minute = remainofhour / 60;
			pIcon->SetHint(finaltext.Format(_AL("%s \rTime remaining: %d hours and %d minutes."), buffstr, hour, minute));
		}
		else if (curVal > 60) {
			int minute = curVal / 60;
			pIcon->SetHint(finaltext.Format(_AL("%s \rTime remaining: %d minutes."), buffstr, minute));
		}
		else
			pIcon->SetHint(buffstr);
		...
		if (pTxt)
		{
			if (curVal != 0 && curVal < 60)
			{
				ACString strNum;
				if (curVal <= 4) {
					pTxt->SetColor(A3DCOLORRGB(255, 0, 0));
				}
				else if (curVal <= 10) {
					pTxt->SetColor(A3DCOLORRGB(255, 15, 15));
				}
				else {
					pTxt->SetColor(A3DCOLORRGB(255, 255, 255));
				}
				pTxt->Show(true);
				pTxt->SetText(strNum.Format(_AL("%d"), curVal - 1));
				POINT pos = pIcon->GetPos(true);
				pTxt->SetPos(pos.x + 8, pos.y + 20);
			}
			else {
				pTxt->Show(false);
			}
...
// =======================================================================
// IconStateBigIcon2IntegersRange �������ֵ��СֵЧ��ͼ��
// =======================================================================
...
CECIconStateMgr::CECIconStateMgr()
{
	...
	RegisterUpdater(new IconStateBigIcon2IntegersRange(203, A3DCOLORRGBA(228, 0, 127, 128)));	//	��������״̬Ч��
	RegisterUpdater(new IconStateBigIcon1Integer(280));											//	˪����״̬Ч��
}
...
void CDlgHost::Resize(A3DRECT rcOld, A3DRECT rcNew){
	//	�����϶�����С��Ȼ���ϴ󣬾�AUInterface���������󣬻ᵼ�´ӣ�0��0�����Ͻ��ܵ�����λ�ã��Ҳ����϶�������Ӱ�����
	//	����ʱ�����·����޸�
	bool xIsZero = (m_x == 0);
	bool yIsZero = (m_y == 0);
