...
const A3DAABB & CECNPCModelDefaultPolicy::GetPickAABB(){	
	...
	void  CECNPCModelDefaultPolicy::UpdateModelAABB()
	{
		if (m_pNPCModel)
			m_pNPCModel->UpdateModelAABB();
	}
	void CECNPCModelDefaultPolicy::TickBeforeAnimate(DWORD dwDeltaTime)
	{
		if (!m_pNPCModel) {
			return;
		}
		...
		m_pNPCModel->SetPos(m_pNPC->GetPos());
		m_pNPCModel->TickBeforeAnimate(dwDeltaTime);
	}
	void CECNPCModelDefaultPolicy::TickAfterAnimate(DWORD dwDeltaTime)
	{
		if (m_pNPCModel)
		{
			m_pNPCModel->TickAfterAnimate(dwDeltaTime, false, true);
			m_pNPCModel->UpdateModelAABB();
		}
	}
	void CECNPCModelDefaultPolicy::TickAnimation(DWORD dwDeltaTime) 
	{
		if (m_pNPCModel)
		{
			m_pNPCModel->TickAnimation(dwDeltaTime);
		}
	}
...
bool CECNPCModelDefaultPolicy::TraceWithBrush(BrushTraceInfo * pInfo)const{	
	...
	//	if (bCollide && !bStartSolid)	//	2012-1-31: ���� bStartSolid=true ʱ����ײ�ᵼ��˲�Ƽ��ܼ���ʧ�ܣ�����ͬ���������� TraceWithBrush ȥ��
	if (bCollide)
	{
		*pInfo = info;
...
const char * CECNPCModelDefaultPolicy::GetActionEnvString()const{
	...
	bool bIsSummonPet = pPet->IsSummonPet();								// �ٻ���
	bool bIsCombatPet = pPet && (pPet->GetDBEssence()->inhabit_type == 6);	// ս����, ��½��
	bool bIsEvoPet    = pPet && (pPet->GetDBEssence()->inhabit_type == 6);	// ������
	if (bIsSummonPet || bIsCombatPet || bIsEvoPet)
	{
		static const char *szEnv[3] = {"����", "ˮ��", "����"};
...
{	
	...
	{
		//	�����˺�������ǰ�����ǡ��˺����ӡ�������ֱ�Ӵ���bon�ж�����ecm�г���
		//	����������ʹ���ˡ��˺���������ecm��bon�ж���
		//	�������µ�NPC�����ٻ��裩�в�ͬ�Ļ�����ˮ½�գ����в�ͬ�Ķ�������
		//	����������±���
...
bool CECNPCModelClonePlayerPolicy::NPCAction2PlayerAction(int iMoveEnv, int iNPCAction, int &iPlayerAction){
	...
	const static int s_playerActionMap[CECNPC::ACT_MAX] = {
		...
		-1,	/*��������*/			/*CECNPC::ACT_ATTACK1*/
		-1,	/*��������*/			/*CECNPC::ACT_ATTACK2*/
		...
		-1,	/*��������*/			/*CECNPC::ACT_NPC_ATTACK*/
		-1,	/*��������*/			/*CECNPC::ACT_NPC_DIE*/
...
void  CECNPCModelClonePlayerPolicy::UpdateModelAABB()
{
}
void CECNPCModelClonePlayerPolicy::TickBeforeAnimate(DWORD dwDeltaTime)
{
	if (!m_pPlayer) {
		return;
	}
...
void CECNPCModelClonePlayerPolicy::TickAfterAnimate(DWORD dwDeltaTime)
{
	m_pPlayer->Tick(dwDeltaTime);
}
void CECNPCModelClonePlayerPolicy::TickAnimation(DWORD dwDeltaTime)
{
}
