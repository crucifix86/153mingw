...
public:
	...
	void OnCommand_VipHelp(const char* szCommand);
	...
	bool IsCashVipShopItem(GSHOP_ITEM* item);
	int GetCashVipItemIndex(int itemIndex); 
	int GetItemIndexFromVipItemIndex(int vipItemIndex);
...
protected:
	...
	PAUISTILLIMAGEBUTTON	m_pVipHelp;
	int						m_playerVipLevel;
	int						m_playerVipScore;
	PAUILABEL				m_pTxt_VipPoints;
	PAUIPROGRESS			m_pPro_VipPro; // Progress bar to use when didnt reach max vip level
	PAUIPROGRESS			m_pPro_VipFull; // Progress bar to use towhen MAX vip level reached
	PAUIIMAGEPICTURE		m_pImg_Attention; // Hint message over Vip progress bar
