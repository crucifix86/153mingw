...
AUI_ON_COMMAND("Btn_CreateChannel",	OnCommand_createchannel)
AUI_ON_COMMAND("joinchannel",	OnCommand_joinchannel)
AUI_ON_COMMAND("Btn_JoinChannel1",	OnCommand_joinchannel1)
...
AUI_ON_COMMAND("Btn_Horn", OnCommand_horn)
...
DWORD CDlgChat::m_dwTickChatGlobal = 0;
...
abase::vector<CDlgChat::CHAT_MSG> CDlgChat::m_vecGlobalMsg;
...
CDlgChat::CDlgChat() : m_pJoinChannelBtn(NULL), m_pCreateChannelBtn(NULL)
{
	m_pObjLock = NULL;
}
...
bool CDlgChat::OnInitDialog()
{
	...
	{
		m_pJoinChannelBtn = (PAUISTILLIMAGEBUTTON)GetDlgItem("Btn_JoinChannel1");
		m_pCreateChannelBtn = (PAUISTILLIMAGEBUTTON)GetDlgItem("Btn_CreateChannel");
		GetDlgItem("Edi_BG02")->Show(false);
		m_pJoinChannelBtn->Show(false);
		m_pCreateChannelBtn->Show(false);
	}
	return true;
}
...
void CDlgChat::OnCommand_speak(const char * szCommand)
{
	...
	if (strText.GetLength() > 2 && strText[0] == '@' && strText[1] == '#')
	{
		int num = GetHostPlayer()->GetPack()->GetItemTotalNum(48179);
		num += GetHostPlayer()->GetPack()->GetItemTotalNum(48178);
		if (num < 1)
		{
			GetGameUIMan()->AddChatMessage(GetStringFromTable(11681), GP_CHAT_MISC);
			return;
		}
		if (CECTimeSafeChecker::ElapsedTime(dwTime, m_dwTickChatGlobal) <= 1000)
		{
			GetGameUIMan()->AddChatMessage(GetStringFromTable(11680), GP_CHAT_MISC);
			return;
		}
	}
	...
	else if (strText.GetLength() >= 2 && strText[0] == '@' && strText[1] == '#')
	{
		...
		ChangeFocus(NULL);
		PAUIDIALOG pMsgBox;
		GetGameUIMan()->MessageBox("Game_ChatGlobal", GetStringFromTable(11682), MB_YESNO, A3DCOLORRGBA(255, 255, 255, 160), &pMsgBox);
...
void CDlgChat::OnCommand_popface(const char * szCommand)
{
	...
	if (m_szName == "Win_Chat" && (IsSuperFarCryMode() || IsGlobalMode()))
	{
		// 号角频道采用单独表情界面
		//
		pShow->Show(false);
...
void CDlgChat::OnCommand_set(const char * szCommand)
{
	...
	PAUITEXTAREA pText;
	PAUIOBJECT pEdit = GetDlgItem("DEFAULT_Txt_Speech");
	ACHAR *a_pszPrefix[] = { _AL(""), _AL("!!"), _AL("!~"), _AL("/"), _AL("$"), _AL("!@"), _AL("!#"), _AL("@#") };

	int sz = sizeof(a_pszPrefix)/sizeof(a_pszPrefix[0]);
	if ( nNewSet < 0 || nNewSet >=  sz)
		nNewSet = 0;
...
abase::vector<CDlgChat::CHAT_MSG>& CDlgChat::GetGlobalMsgs()
{
	return m_vecSuperFarCryMsg;
}
...
void CDlgChat::OnEventLButtonUp_Face(WPARAM wParam, LPARAM lParam, AUIObject *pObj)
{
	...
	if (m_szName == "Win_Popface01") {
		if (IsSuperFarCryMode()) {
			nEmotion = SUPER_FAR_CRY_EMOTION_SET;
		}
		else if (IsGlobalMode()) {
			nEmotion = GLOBAL_EMOTION_SET;
		}
	}
...
void CDlgChat::OnCommand_joinchannel1(const char* szCommand)
{
...
void CDlgChat::OnCommand_joinchannel(const char * szCommand)
{
	...
	{
		GetDlgItem("Edi_BG02")->Show(true);
		m_pCreateChannelBtn->Show(true);
		m_pJoinChannelBtn->Show(true);
	}
}
void CDlgChat::OnCommand_horn(const char* szCommand)
{
	if (m_pAllianceBtn->IsShow())
	{
		GetDlgItem("Edi_BG03")->Show(false);
		m_pAllianceBtn->Show(false);
		((PAUISTILLIMAGEBUTTON)GetDlgItem("Btn_Set7"))->Show(false);
	}
	else
	{
		GetDlgItem("Edi_BG03")->Show(true);
		m_pAllianceBtn->Show(true);
		m_pAllianceBtn->Enable(false);
		((PAUISTILLIMAGEBUTTON)GetDlgItem("Btn_Set7"))->Show(true);
	}
}
	...
	if( stricmp(m_szName, "Win_Popface") == 0)
	{
		...
		if (pDlg)
		{
			...
			if (IsShow() &&
				...
				(IsSuperFarCryMode() || IsGlobalMode()))
			{
				// 切换使用号角频道专用表情
