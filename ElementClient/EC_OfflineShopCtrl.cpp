...
const __int64 MAX_MONEY_INSHOP = 100000000000; // �ܼ�����
const int YINPIAO = 10000000; // ��Ʊ
const __int64 MAX_UNIT_PRICE = 4000000000;// ��������
const int MAX_MONEY_INVENTORY = 2000000000;// ������Ǯ����
//////////////////////////////////////////////////////////////////////////
//
//
//
...
void COfflineShopCtrl::OnServerNotify(int iProtocol, GNET::Protocol* pProtocol)
{
	...
	case PROTOCOL_PSHOPCLEARGOODS_RE:
		...
		break;
	case PROTOCOL_PSHOPSELFGET_RE: // ��ȡ�Լ�������Ϣ
		{
			PShopSelfGet_Re* p = dynamic_cast<PShopSelfGet_Re*>(pProtocol);			
			
			if (p->retcode == ERR_SUCCESS)
			{
				if (m_iNPCSevFlag == NPCSEV_SETTING) //ȡ���Լ�������Ϣ
				{					
			...
			else if (p->retcode == ERR_PLAYERSHOP_NOTFIND) // û�з��ֵ���
			{				
				CDlgOfflineShopCreate* pDlg = dynamic_cast<CDlgOfflineShopCreate*>(m_pGameUIMan->GetDialog("Win_ShopCreate"));
	...
	case PROTOCOL_PSHOPLIST_RE:
		{
			...
			for(;i<(int)p->shoplist.size();i++)
			{
				...
				shop.invState = p->shoplist[i].invstate; // 0x01:�������ǿ�;0x02:�չ����ǿ�
				shop.bNameOK = false;
		...
		break;
	case PROTOCOL_PSHOPLISTITEM_RE: // ��ҳ����һ��
		{
...
bool COfflineShopCtrl::CheckMoneyStore(int yinpiao, int money) // ��Ǯ���
{
