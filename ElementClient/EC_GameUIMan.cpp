...
bool CECGameUIMan::LoadIconSet()
{
	...
	GNET::RoleInfo Info = g_pGame->GetGameRun()->GetSelectedRoleInfo();
	char a_szIconFile[ICONS_MAX][40] = { "Action", "Skill", "Ivtr", "State", "SkillGrp", "Guild", "Pet", "ELF", "Suite","ChangeEqp", "Calendar", "PQ" };
...
bool CECGameUIMan::DealWindowsMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if( m_pDlgMouseOn && !m_pMapDlgsMgr->IsMarking() &&
		...
		&& !m_pDlgGeneralCard->IsSwallowing() && !IsCustomizeCharacter() && !m_pDlgAstrolabe->IsSwallowing() )
	{
	...
	if( m_bRepairing || m_pDlgGeneralCard->IsSwallowing() || m_pDlgAstrolabe->IsSwallowing())
	{
		if( uMsg == WM_MOUSEMOVE )
			bRet = true;
		else if( uMsg == WM_RBUTTONDOWN )
		{
			...
			m_pDlgAstrolabe->SetSwallowing(false);
			g_pGame->ChangeCursor(RES_CUR_NORMAL);
			bRet = true;
		}
		else
...
bool CECGameUIMan::OnCommand(const char* szCommand, AUIDialog* pDlg)
{
	...
	if( 0 == stricmp(pDlg->GetName(), "Win_Popmsg") ||
		0 == stricmp(pDlg->GetName(), "Win_Popmsg3") ||
...
bool CECGameUIMan::OnMessageBox(int iRetVal, AUIDialog* pDlg)
{
	...
	*/
	...
	else if (0 == stricmp(pDlg->GetName(), "Game_ChatGlobal"))
	{
	int nPack = (int)pDlg->GetData();
	int nSlot = (int)pDlg->GetDataPtr();
	...
	if (iRetVal == IDYES || iRetVal == IDOK)
	{
		pSession->SendChatData(GP_CHAT_GLOBAL, pDlg->GetText(), nPack, nSlot);
		m_pDlgChat->m_dwTickChatGlobal = GetTickCount();
		cm.dwTime = m_pDlgChat->m_dwTickChatGlobal;
	}
	else
		cm.dwTime = 0;
	cm.cChannel = GP_CHAT_GLOBAL;
	cm.strMsg = ACString(_AL("@#")) + pDlg->GetText();
	cm.strMsg.CutRight(8); // ɾ�����ӵ���ɫ�������Ϣ
	m_pDlgChat->m_vecHistory.push_back(cm);
	if (m_pDlgChat->m_vecHistory.size() >= 20)
		m_pDlgChat->m_vecHistory.erase(m_pDlgChat->m_vecHistory.begin());
	m_pDlgChat->m_nCurHistory = m_pDlgChat->m_vecHistory.size();
	...
	if (!stricmp(pDlg->GetName(),"Game_NewbieGift") && iRetVal == IDYES)
	{
	...
	else if (!stricmp(pDlg->GetName(), "Game_Stargaze_Use") && IDOK == iRetVal)
	{
		m_pDlgAstrolabe->OnConfirmStargaze(pDlg->GetData("ID"));
	}
	else if (!stricmp(pDlg->GetName(), "Game_Swallow_Use") && IDOK == iRetVal)
	{
		m_pDlgAstrolabe->ShowSwallowDialog((AstrolabeMsg*)pDlg->GetDataPtr("SwallowItem"));
	}
	else if (!stricmp(pDlg->GetName(), "Game_Swallow_DiscardHigher") && IDOK == iRetVal)
	{
		m_pDlgAstrolabeSwallow->SendSwallowCmd((CECIvtrAstrolabe*)pDlg->GetDataPtr("KeepItem"));
		m_pDlgAstrolabeSwallow->Show(false);
	}
	else if (!stricmp(pDlg->GetName(), "Game_Starshift_Use") && IDOK == iRetVal)
	{
		m_pDlgAstrolabeIncPoint->OnConfirmShift();
	}
	...
	else if (0 == stricmp(pDlg->GetName(), "Game_RemoteRepair") && IDOK == iRetVal) // This is VIP service: Remote Repair
	{
		pHost->CmdRemoteRepair();
	}
	else if (0 == stricmp(pDlg->GetName(), "FixPositionDeleteConfirm") && IDOK == iRetVal) // Quick Teleport : Delete position
	{
		m_pDlgMarkVip->DoDelete();
	}
	else if (0 == stricmp(pDlg->GetName(), "FixPositionTeleportConfirm") && IDOK == iRetVal) // Quick Teleport : Teleport 
	{
		m_pDlgMarkVip->DoTeleport();
	}
	else if (0 == stricmp(pDlg->GetName(), "CashResurrect") && IDOK == iRetVal) // Cash Resurrect
	{
		m_pDlgRevive->DoCashResurrect();
	}
...
//		m_pDlgAutoHelp->IncOpenTime(dwDeltaTime);
	...
	if (IsNeedShowVipInfo()) 
	{
		int vipScore = g_pGame->GetGameRun()->GetHostPlayer()->GetVipScore();
		int vipLevel = g_pGame->GetGameRun()->GetHostPlayer()->GetVipLevel();
		ACString vipInfoStr;
		AddChatMessage(vipInfoStr.Format(GetStringFromTable(11751), vipLevel, vipScore, CECVipConfig::Instance().GetVipScoreReducePerDayByLevel(vipLevel)), GP_CHAT_SYSTEM);
		m_bVipInfoShown = true;
	}
...
//	}
	...
	else if (CDlgChat::IsGlobalMode()) {
		pEmotionEdit->SetCurrentEmotion(GLOBAL_EMOTION_SET);
		nCurSet = GLOBAL_EMOTION_SET;
	...
	char cChannel, int idPlayer, const ACHAR *pszPlayer, char byFlag, char cEmotion, CECIvtrItem *pItem,const ACHAR *pszMsgOrigion)
{
	...
	else
	{
		...
		if (cChannel == GP_CHAT_SUPERFARCRY || cChannel == GP_CHAT_GLOBAL)
		{
...
void CECGameUIMan::SetMarqueeMsg3(const ACHAR* pszMsg)
{
	PAUIDIALOG pDlg = GetDialog("Win_Popmsg3");
	PAUILABEL pLabel = (PAUILABEL)pDlg->GetDlgItem("Message");
	pLabel->SetCurrentEmotion(GLOBAL_EMOTION_SET);
	pLabel->SetText(pszMsg);
	pLabel->SetMarquee(true, 2, 120);
...
bool CECGameUIMan::UpdateTask(unsigned long idTask, int reason)
{
	...
	if (reason == TASK_SVR_NOTIFY_NEW)
	{
		...
		if (pMan->IsTowerTask(idTask)) {
			CDlgSoloTowerChallengeState* pDlg = dynamic_cast<CDlgSoloTowerChallengeState*>(GetDialog("Win_SoloTowerChallengeState"));
			pDlg->SetChallengeTask(idTask);
		}
	}
...
bool CECGameUIMan::UpdateHint()
{
	...
	else if (dynamic_cast<CDlgAstrolabeIncPointValue*>(pDlg) != NULL && dynamic_cast<AUIImagePicture*>(pObj) ) {
		if (strstr(pObj->GetName(), "Item_")) {
			// 装备
			CECIvtrItem* pItem = (CECIvtrItem*)pObj->GetDataPtr("ptr_CECIvtrItem");
			if (pItem) {
	...
	else if (dynamic_cast<CDlgAstrolabeRandomAddon*>(pDlg) != NULL && dynamic_cast<AUIImagePicture*>(pObj)) {
		if (strstr(pObj->GetName(), "Item_")) {
			// 装备
			CECIvtrItem* pItem = (CECIvtrItem*)pObj->GetDataPtr("ptr_CECIvtrItem");
			if (pItem) {
...
void CECGameUIMan::EnableUI(bool bEnable)
{
	...
	CDlgQuickBar* pQuickBar = dynamic_cast<CDlgQuickBar*>(GetDialog(dlgName));
	pQuickBar->extend_bar1(g_pGame->GetConfigs()->GetGameSettings().bExtendedSkillbar1);
	pQuickBar->extend_bar2(g_pGame->GetConfigs()->GetGameSettings().bExtendedSkillbar2);
	g_bRenderNoFocus = g_pGame->GetConfigs()->GetGameSettings().bUnfreeze;
}
...
void CECGameUIMan::UpdateAstrolabe(S2C::cmd_astrolabe_operate_result* cmd)
{
	m_pDlgAstrolabe->NeedsUpdate(true, cmd);
}
...
void CECGameUIMan::RearrangeWindows(A3DRECT rcOld, A3DRECT rcNew)
{
	...
	PAUIDIALOG pDlgPopmsg3 = GetDialog("Win_Popmsg3");
	if (pDlgPopmsg3->IsShow())
		pDlgPopmsg3->SetPosEx(0, GetDialog("Win_HpmpOther")->GetSize().cy, AUIDialog::alignCenter);
...
void CECGameUIMan::CalcWindowScale()
{
	...
	if (!pConfig || pConfig->GetSystemSettings().bScaleUI)
	{
		...
		a_Clamp(m_fWindowScale, 0.78125f, 1.5f);
	}
	else
	{
		// 否则，保持不缩放
...
bool CECGameUIMan::IsNeedShowVipInfo()
{
	if (!m_bVipInfoShown)
	{
		return g_pGame->GetGameRun()->GetHostPlayer()->GetVipLevel() > 0;
	}
	return false;
}
...
PAUIDIALOG CECGameUIMan::GetActiveChatDialog()
{
	...
	// hopefully this is what was done....
	for(int i = 0; i<m_DlgZOrder.GetCount(); i++)
	{
		pDlg = m_DlgZOrder.GetByIndex(i);
...
void CECGameUIMan::RefreshQuickBarName()
{
	...
	QUICKBAR_REFRESH("Win_Quickbar13Ha", 25, 9);
	QUICKBAR_REFRESH("Win_Quickbar13Va", 25, 9);
	QUICKBAR_REFRESH("Win_Quickbar12Ha", 35, 8);
	QUICKBAR_REFRESH("Win_Quickbar12Va", 35, 8);
}
