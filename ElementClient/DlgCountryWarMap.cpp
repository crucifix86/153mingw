...
static const char*			FLAG_PATH = "number\\����.tga";
static const char*			FLAG_DEST_PATH = "Number\\ս�����찴ť.tga";

static const char*			SKULL_PATH[2] = {"ͼ��\\kulou_offense.dds", "ͼ��\\kulou_defence.dds"};
...
AString CDlgCountryWarMap::GetRankImage(bool offense, int rank)
{
	AString ret;
	if (rank >= 1 && rank < 6)
		ret.Format("����\\%s%d.gfx", offense ? "offense" : "defence", rank);
	return ret;
}
