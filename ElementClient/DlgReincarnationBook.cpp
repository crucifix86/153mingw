...
void CDlgReincarnationBook::Update()
{
	...
	for (i = 0; i < tome.reincarnations.size(); ++i) {
		...
		if (pObj) {
			pObj->Show(true);
			pObj->Enable(tome.reincarnations[i].level < g_GameCfgs.GetPlayerMaxLevel());
		}
		if (tome.reincarnations[i].level < g_GameCfgs.GetPlayerMaxLevel()) bReach105EveryTime = false;
	}
	while (true){
		strTemp.Format("Txt_Reincar%d", i + 1);
		pObj = GetDlgItem(strTemp);
...
void CDlgReincarnationBook::UpdateSleepWakeBtn(AUIObject* pObj) {
	...
	for (unsigned int i = 0; i < tome.reincarnations.size(); ++i) {
		if (tome.reincarnations[i].level < g_GameCfgs.GetPlayerMaxLevel()) {
	...
	if (pObj) {
		...
		if (iLevel<100)
			...
			bEnable = true;
			if (iLevel >= g_GameCfgs.GetPlayerMaxLevel())
