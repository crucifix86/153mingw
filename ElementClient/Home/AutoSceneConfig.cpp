...
bool CAutoSceneConfig::LoadTreeTypeList(const char* szFile)
{
	...
	// Read tree infos
	for (int i=0; i<m_nNumAutoTreeTypeList; i++)
	{
		// Read ID
		if (!fileToLoad.ReadLine(szLine, 256, &dwReadLen))
			goto READFAIL;
...
bool CAutoSceneConfig::LoadGrassTypeList(const char* szLandGrassFile, const char* szWaterGrassFile)
{
	...
	// Read grass infos
	for (int i=0; i<m_nNumAutoLandGrassTypeList; i++)
	{
		// Read ID
		if (!fileToLoad.ReadLine(szLine, 256, &dwReadLen))
			goto READFAIL;
	...
	// Read grass infos
	for (int i=0; i<m_nNumAutoWaterGrassTypeList; i++)
	{
		// Read ID
		if (!fileToLoad.ReadLine(szLine, 256, &dwReadLen))
			goto READFAIL;
...
bool CAutoSceneConfig::LoadTextureTypeList(const char* szFile)
{
	...
	// Read texture infos
	for (int i=0; i<m_nNumAutoTextureTypeList; i++)
	{
		// Read ID
		if (!fileToLoad.ReadLine(szLine, 256, &dwReadLen))
			goto READFAIL;
...
bool CAutoSceneConfig::LoadModelTypeList(const char* szFile)
{
	...
	// Read model infos
	for (int i=0; i<m_nNumAutoModelTypeList; i++)
	{
		// Read ID
		if (!fileToLoad.ReadLine(szLine, 256, &dwReadLen))
			goto READFAIL;
...
bool CAutoSceneConfig::LoadSkyTypeList(const char* szFile)
{
	...
	// Read sky infos
	for (int i=0; i<m_nNumAutoSkyTypeList; i++)
	{
		// Read ID
		if (!fileToLoad.ReadLine(szLine, 256, &dwReadLen))
			goto READFAIL;
