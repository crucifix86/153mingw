...
bool CAutoHillPoly::CalArea()
{
	...
	int nBottom = -1;
	int i;
	for (int i=0; i<m_Area.GetNumPoints(); i++)
	{
...
bool CAutoHillPoly::GetMountainBridge()
{
	...
	int nPos;
	float fCosValue;
	for (int i=0; i<nSalientPoints; i++)
	{
		fTotalCos = 0;
...
void CAutoHillPoly::ExpandPoints(int x, int y, float fExpandDis, float* pNeedDraw)
{
	...
	for (int i=y-nMaxDis; i<=y+nMaxDis; i++)
	{
