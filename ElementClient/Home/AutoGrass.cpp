...
{
	int i;
	for (int i=0; i<m_aBitGrassMap.GetSize(); i++)
	{
		SAFERELEASE(m_aBitGrassMap[i]);
	}
	m_aBitGrassMap.RemoveAll();
	for (int i=0; i<m_aHasGrassInMap.GetSize(); i++)
	{
...
{
	int i;
	for (int i=0; i<m_aGrassAddedTypeIndex.GetSize(); i++)
	{
...
bool CAutoGrass::RemoveAllGrass()
{
	...
	for (int i=0; i<m_aPlantedGrassID.GetSize(); i++)
	{
