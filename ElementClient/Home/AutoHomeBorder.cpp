...
bool CAutoHomeBorder::Init()
{
	...
	for (int i=NUM_HOME_BORDER_POINT; i<=(m_nNumInside+1)*2-1; i++)
	{
	...
	for (int i=(m_nNumInside+1)*2+1; i<=(m_nNumInside+1)*3-1; i++)
	{
		m_pBorderPoints[i].nType = BPT_ROW;
	...
	for (int i=(m_nNumInside+1)*3+1; i<=(m_nNumInside+1)*4-1; i++)
	{
		m_pBorderPoints[i].nType = BPT_COL;
...
bool CAutoHomeBorder::Render()
{
	...
	for (int i=0; i<m_nNumPoints-1; i++)
	{
...
{
	int i;
	for (int i=0; i<NUM_HOME_BORDER_POINT; i++)
	{
...
bool CAutoHomeBorder::GetHomeLeftPoints(float* pPoints)
{
	...
	for (int i=1; i<NUM_HOME_BORDER_POINT; i++)
	{
