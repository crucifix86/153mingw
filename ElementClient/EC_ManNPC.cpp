...
bool CECNPCMan::Tick(DWORD dwDeltaTime)
{
	...
	//	Tick all NPCs
	CECParallelTickOnlyAnimationManager<CECNPC*> elems;
	...
	for (; it != m_NPCTab.end(); ++it)
	{
		...
		{
			elems.AddElem(pNPC, dwDeltaTime);
			pNPC->TickBeforeAnimate(dwDeltaTime);
		}
	}
	...
	{
		CECNPC* pNPC = *it.value();
		pNPC->TickAfterAnimate(dwDeltaTime);
	}
...
// BOLA NOTES
static const char* szGFXFile = "npc\\����_������Ʒ.gfx";
//	Render routine
bool CECNPCMan::Render(CECViewport* pViewport)
{
	...
	if (pHost)
	{
		...
		idSelTarget = pHost->GetSelectedTarget();
	}
	...
	for (; it != m_NPCTab.end(); ++it)
	{
		...
		if (idHoverObject == pNPC->GetNPCID())
			pHHNPC = pNPC;
		else if (pNPC->GetHighlight())
			pNPC->RenderHighLight(pViewport);
		else if (pNPC->GetNPCID() == idSelTarget)
			pNPC->RenderHighLight(pViewport);
		else
			pNPC->Render(pViewport);

		if (pNPC->HasModel()) {
			// && pNPC->GetNPCID() != idSelTarget // to hide GFX when player selects it
			if (pNPC->IsRequiredForTask() && !pNPC->IsDead()) {
				if (!pNPC->IsRenderingTaskGFX()) {
					pNPC->PlayGfx(szGFXFile, NULL);
					pNPC->SetRenderingTaskGFX(true);
				}
			}
			else {
				// pNPC->GetNPCID() == idSelTarget || // to hide GFX when player selects it
				if (pNPC->IsRenderingTaskGFX() || pNPC->IsDead()) {
					pNPC->RemoveGfx(szGFXFile, NULL);
					pNPC->SetRenderingTaskGFX(false);
				}
			}
		}
	}
...
void CECNPCMan::NPCDisappear(int nid)
{
	...
	if (pNPC)
	{
		if (!pNPC->IsDead())
		{
			//	NPC ��ʧʱ���������ϻ��������Ч����Ҫ���������Ա�ʱ��ը��Ч��
			//	��Ȼ�� CECNPC::Killed ���������������ͻ��˿��ܻ�ֱ���յ� disappear ��Ϣ�����Ա����ܣ�
			//	���������Ҫ���Ӵ�������
			//	��֮ǰ NPC ����������˵���ѵ��ù�
...
bool CECNPCMan::NPCEnter(const S2C::info_npc& Info, bool bBornInSight)
{
	...
	//	NPC �������������Ҹ���ʱ��С��5��ʱ���л���ָ�뻹�� m_NPCTab �� �� m_aMMNPCs ��
	//	��ˣ���������ʱ���ᵥ��ɾ�� m_NPCTab ��� NPC ���� m_aMMNPCs ��������Чָ�룬
	//	��ǡ�ɡ�ͬʱ������ͬһ Tick �����Χ���� NPC ��ʧ����Ϣ��Ҫ����������ǡ����Чָ���� m_aMMNPCs ������һ����
	//	��������£����ڴ� m_aMMNPCs ��ɾ�����������NPC��ʱд������Чָ�롢�Ӷ����¿ͻ��˱���
