...
//		m_szModelFile	= pData->file_matter;  //  name...
	...
	{
		const ASTROLABE_ESSENCE* pData = (const ASTROLABE_ESSENCE*)pDataPtr;
		m_szName = pData->name;
		m_szModelFile = pData->file_matter;
		break;
	}
	...
	{
		const ITEM_PACKAGE_BY_PROFESSION_ESSENCE* pData = (const ITEM_PACKAGE_BY_PROFESSION_ESSENCE*)pDataPtr;
		m_szName = pData->name;
		m_szModelFile = pData->file_matter;
		break;
	}
	...
	{
		const ASTROLABE_INC_EXP_ESSENCE* pData = (const ASTROLABE_INC_EXP_ESSENCE*)pDataPtr;
		m_szName = pData->name;
		m_szModelFile = pData->file_matter;
		break;
	}
	...
	{
		const ASTROLABE_INC_INNER_POINT_VALUE_ESSENCE* pData = (const ASTROLABE_INC_INNER_POINT_VALUE_ESSENCE*)pDataPtr;
		m_szName = pData->name;
		m_szModelFile = pData->file_matter;
		break;
	}
	...
	{
		const ASTROLABE_RANDOM_ADDON_ESSENCE* pData = (const ASTROLABE_RANDOM_ADDON_ESSENCE*)pDataPtr;
		m_szName = pData->name;
		m_szModelFile = pData->file_matter;
		break;
	}
	...
	{
		const FIX_POSITION_TRANSMIT_ESSENCE* pData = (const FIX_POSITION_TRANSMIT_ESSENCE*)pDataPtr;
		m_szName = pData->name;
		m_szModelFile = pData->file_matter;
		break;
	}
	...
	{
		const FIREWORKS2_ESSENCE* pData = (const FIREWORKS2_ESSENCE*)pDataPtr;
		m_szName = pData->name;
		m_szModelFile = pData->file_matter;
		break;
	}
...
//	Tick routine
bool CECMatter::TickOnlyAnimation(DWORD dwDeltaTime)
{
	if (m_pModel)
		m_pModel->TickAnimation(dwDeltaTime);
	if (m_pGfx)
		m_pGfx->TickAnimation(dwDeltaTime);
	return true;
}
bool CECMatter::TickBeforeAnimate(DWORD dwDeltaTime)
{
	if (m_pModel)
		m_pModel->TickBeforeAnimate(dwDeltaTime);
	return true;
}
bool CECMatter::TickAfterAnimate(DWORD dwDeltaTime)
{
	...
	if (m_pModel)
	{
		m_pModel->TickAfterAnimate(dwDeltaTime, false, true);
		m_pModel->UpdateModelAABB();
	}
	return true;
}
