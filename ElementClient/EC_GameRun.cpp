...
struct EC_STATISTIC
{
	...
	int		iRenderWorldTime;	//	Total render world time

	int		iRenderGfxCasterTime;
	int		iRenderGfxSkinTime;
	int		iRenderAllGfxTime;
	int		iRenderTextTime;
	int		iRenderUITime;
	//	Object counter
	int		iNumPlayer;			//	Else player number
	int		iNumPlayerCache;	//	Else player cache size
...
static int			l_iDelayTimeStamp;	//	��һ�η��������ӳٲ�ѯЭ��ʱ��ʱ���
static CECCounter	l_DelayQueryCounter;//	��ѯ�����ӳ�Э��ļ�ʱ��

//	�����ʱ
static CECCounter	l_QueryServerTime;	//	��ѯ��������ǰʱ��
...
//	sprintf(szFile, "maps\\132\\132.ecwld");
	if (!m_pWorld->LoadWorld(szFile, vHostPos))
	{
		...
		return true;
			...
			if(pPlayer){
				if(pPlayer->GetCameraOnFocus())	{		
	    			pPlayer->SetVisible(true);
					pPlayer->SetRenderNameFlag(true);
					pPlayer->SetCameraOnFocus(false);
				}
			}
			// Release Camera For any reason
			A3DCamera * pCamera = g_pGame->GetViewport()->GetA3DCamera();
			pCamera->UnlockPos();
			pCamera->UnlockDirAndUp();
...
//	m_pNormalSCS->SetShortcut(6, pSC);
	...
	pSC = new CECSCCommand(CECSCCommand::CMD_PICKUPALL);
	m_pNormalSCS->SetShortcut(8, pSC);
	pSC = new CECSCCommand(CECSCCommand::CMD_QUICKTELEPORT);
	m_pNormalSCS->SetShortcut(9, pSC);
...
CECSCCommand* CECGameRun::GetCmdShortcut(int iCommand)
{
	...
	ASSERT(m_pNormalSCS);
	for (int i=0; i < m_pNormalSCS->GetShortcutNum(); i++)
	{
	...
	ASSERT(m_pTeamSCS);
	for (int i=0; i < m_pTeamSCS->GetShortcutNum(); i++)
	{
	...
	ASSERT(m_pTradeSCS);
	for (int i=0; i < m_pTradeSCS->GetShortcutNum(); i++)
	{
	...
	for (int i=0; i < m_pFactionSCS->GetShortcutNum(); i++)
	{
...
{	
	...
	//	Tick GFX Manager
	CECParallelECMGFXTickAnimationManager<A3DGFXEx*> elems;
	std::vector<A3DGFXEx*> gfxToTick;
	g_pGame->GetA3DGFXExMan()->Tick(dwDeltaTime, &gfxToTick);
	elems.AddAll(gfxToTick);
	elems.TickAnimation(dwDeltaTime);
	...
	if ((dwEarTime += dwDeltaTime) >= TIME_UPDATEEAR)
	{
		...
		if (GetGameState() == CECGameRun::GS_GAME && pHostPlayer && pHostPlayer->HostIsReady())
		{
			...
			if (pAM3DSoundDevice)
			{
				A3DVECTOR3 soundPos = pHostPlayer->GetPos() + A3DVECTOR3(0.0f, 0.8f, 0.0f);
				pAM3DSoundDevice->SetPosition(soundPos);
...
#else
		// GCC doesn't support SEH, use regular exception handling
		try
		{
			CheckDefenceCodeImpl();
		}
...
bool CECGameRun::Render(bool bPresent)
{
	...
	//	Render world
	m_dwRenderWorldTime = a_GetTime();
	if (m_pWorld && m_pWorld->IsValid())
		m_pWorld->Render(pViewport);
	
	m_dwRenderWorldTime = a_GetTime() - m_dwRenderWorldTime;
	l_Statistic.iRenderWorldTime = m_dwRenderWorldTime;
	{
		PROFILE_SCOPE("Gfx");
		l_Statistic.iRenderGfxCasterTime = a_GetTime();
		//	Render GFX
		g_pGame->GetGFXCaster()->Render(pViewport);
		pA3DEngine->RenderGFX(pViewport->GetA3DViewport(), 0xffffffff);
		l_Statistic.iRenderGfxCasterTime = a_GetTime() - l_Statistic.iRenderGfxCasterTime;

		if (g_pA3DConfig->RT_GetShowBoundBoxFlag())
			g_pGame->GetA3DGFXExMan()->RenderAllGfxAABB();

		l_Statistic.iRenderGfxSkinTime = a_GetTime();
		g_pGame->GetA3DGFXExMan()->RenderAllSkinModels(pViewport->GetA3DViewport());
		A3DSkinRender* psr1 = g_pGame->GetSkinRender1();

		l_Statistic.iRenderGfxSkinTime = a_GetTime() - l_Statistic.iRenderGfxSkinTime;
		l_Statistic.iRenderAllGfxTime = a_GetTime();
		...
		l_Statistic.iRenderAllGfxTime = a_GetTime() - l_Statistic.iRenderAllGfxTime;
	}
	...
	{
		PROFILE_SCOPE("Text");
		l_Statistic.iRenderTextTime = a_GetTime();
		...
		l_Statistic.iRenderTextTime = a_GetTime() - l_Statistic.iRenderTextTime;
	}
	...
	{
		PROFILE_SCOPE("UI");
		l_Statistic.iRenderUITime = a_GetTime();
		//	Render 2D gfx
		...
		l_Statistic.iRenderUITime = a_GetTime() - l_Statistic.iRenderUITime;
	}
...
void CECGameRun::DisplayGameStatistic()
{
	...
	if (l_StatCnt.IsFull())
	{
		...
		l_Statistic.iNumPlayerCache = m_pWorld->GetPlayerMan()->GetCacheSize();
	...
	a_sprintf(szMsg, _AL("player cache: %d"), l_Statistic.iNumPlayerCache);
	pA3DFont->TextOut(x, y, szMsg, dwCol);
	y += 16;
	...
	pA3DFont->TextOut(x, y, szMsg, dwCol);
	y += 16;
	a_sprintf(szMsg, _AL("render world time: %d"), l_Statistic.iRenderWorldTime);
	pA3DFont->TextOut(x, y, szMsg, dwCol);
	y += 16;
	a_sprintf(szMsg, _AL("render gfx caster: %d"), l_Statistic.iRenderGfxCasterTime);
	pA3DFont->TextOut(x, y, szMsg, dwCol);
	y += 16;
	a_sprintf(szMsg, _AL("render gfx skin: %d"), l_Statistic.iRenderGfxSkinTime);
	pA3DFont->TextOut(x, y, szMsg, dwCol);
	y += 16;
	a_sprintf(szMsg, _AL("render all gfx: %d"), l_Statistic.iRenderAllGfxTime);
	pA3DFont->TextOut(x, y, szMsg, dwCol);
	y += 16;
	a_sprintf(szMsg, _AL("render text: %d"), l_Statistic.iRenderTextTime);
	pA3DFont->TextOut(x, y, szMsg, dwCol);
	y += 16;
	a_sprintf(szMsg, _AL("render UI: %d"), l_Statistic.iRenderUITime);
	pA3DFont->TextOut(x, y, szMsg, dwCol);
	y += 16;
...
*/	case MSG_EXITGAME:
	...
	case MSG_INSTCHECKOUT:
	{
		...
		m_nGShopTimeStamp3 = pCmd->gshop_time_stamp3;
		if (m_nGShopTimeStamp3 != globaldata_getgshop_timestamp3())
		{
			AddFixedMessage(FIXMSG_ERR_GSHOPDATA);
			a_LogOutput(1, "gshop2 timestamp error:server(0x%x) != local(0x%x)", m_nGShopTimeStamp3, globaldata_getgshop_timestamp3());
		}
	...
	case MSG_CASHVIPMALLITEMINFO:
	{
		if (Msg.dwParam2 == S2C::CASH_VIP_MALL_ITEM_PRICE) {
			const S2C::cmd_cash_vip_mall_item_price* pCmd = (const S2C::cmd_cash_vip_mall_item_price*)Msg.dwParam1;
			if (CECVipShop::Instance().UpdateFromServer(pCmd)) {
				CECGameUIMan* pGameUI = g_pGame->GetGameRun()->GetUIManager()->GetInGameUIMan();
				((CDlgQShop*)(pGameUI->GetDialog("Win_QShop")))->UpdateQshopData();
			}
		}
		else if (Msg.dwParam2 == S2C::CASH_VIP_MALL_ITEM_BUY_RESULT) {
			const S2C::cmd_cash_vip_mall_item_buy_result* pCmd = (const S2C::cmd_cash_vip_mall_item_buy_result*)Msg.dwParam1;
			if (pCmd->result == 1) // Error
				CECVipShop::Instance().GetFromServer(pCmd->index, pCmd->index + 1);
		}
		break;
	}

	case MSG_PURCHASELIMITINFO:
	{
		const S2C::cmd_purchase_limit_info_notify* pCmd = (const S2C::cmd_purchase_limit_info_notify*)Msg.dwParam1;
		
		// Note: If we really want (in the future) to implement this system for all gshops, we should send a value indicating which shop is affected.
		// Otherwise, we should be guessing by active shop when the answer is received and it doesnt seem accurate.
		// Since this is just for VIP only by now, I think it will be ok to handle this way.

		a_LogOutput(1, "cmd_purchase_limit_info_notify count=%d ", pCmd->count);
		if (pCmd->count > 0) {
			CECGameUIMan* pGameUI = g_pGame->GetGameRun()->GetUIManager()->GetInGameUIMan();
			// Update info for limited items received
			for (int i = 0; i < pCmd->count; i++) {
				//a_LogOutput(1, "cmd_purchase_limit_info_notify (%d) limit_type=%d, item_id=%d, have_purchased_count=%d ", i, pCmd->list[i].limit_type, pCmd->list[i].item_id, pCmd->list[i].have_purchase_count);
				// Get Base Shop
				CECShopBase& baseShop = (CECShopBase&) CECVipShop::Instance(); // In the case of multiple shop limits, we should select the right shop based on a new parameter "shop_type"
				baseShop.SetLimitedItem(pCmd->list[i].limit_type, pCmd->list[i].item_id, pCmd->list[i].have_purchase_count);
			}
			// Update Qshop UI and related.
			((CDlgQShop*)(pGameUI->GetDialog("Win_QShop")))->UpdateQshopData();
		...
		else if (Msg.dwParam2 == S2C::PUBLIC_QUEST_RANKS)
		{
			...
			for(int i=0;i<sizeof(pCmd->ranks_size)/sizeof(pCmd->ranks_size[0]);i++)
			{
				...
				for(int j=0; j<pCmd->ranks_size[i]; j++)
				{
					...
					rankInfo.rand_score	= re.rand_score;	// Ĭ��Ϊ-1
	...
	case MSG_RANDOM_MAP_ORDER:
		{
			...
			/*	CECRandomMapProcess* pRandom = m_pWorld->GetRandomMapProc();
				if (pRandom)
				{
					...
					m_pWorld->SetRandomMapFlag(true);// ʲôʱ������Ϊfalse
				}*/
			}
			break;
		}
	case MSG_SCENE_SERVICE_NPC_LIST:
		{
			S2C::cmd_scene_service_npc_list* npcList = (S2C::cmd_scene_service_npc_list*)Msg.dwParam1;

			// Check if we received the NPC that is required to teleport in Solo Challenge
			if (npcList->count > 0) {
				for (unsigned int i = 0; i < npcList->count; i++) {
					if (npcList->list[i].tid == 47576) {
						CDlgSoloTowerChallengeSelectLayer* pDlg = (CDlgSoloTowerChallengeSelectLayer*)GetUIManager()->GetInGameUIMan()->GetDialog("Win_SoloTowerChallengeSelectLayer");
						if (pDlg)
						{
							pDlg->SetServiceNPCNID(npcList->list[i].nid);
						}
					}
				}
			}
