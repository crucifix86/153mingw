...
void CECBaseUIMan::OptimizeSpriteResource(bool bOptimize)
{
	for( int i = 0; i < (int)m_aDialog.size(); i++ )
	{
		...
		if (!bOptimize){
			pDlg->OptimizeSpriteResource(true);			//	����������Դ
		}else if (!pDlg->IsShow()){
			pDlg->OptimizeSpriteResource(false);		//	��û��ʾ��Դж����Դ
		}
	}
}
...
bool CECBaseUIMan::RenderUIDebug()
{
	...
	// judge the size
	x = max(0, static_cast<int>(ptPos.x));
	y = ptPos.y + 32;
...
{
	//	szWinExplorer:	���� IE ������ڵ����ƣ�Ϊ NULL ��ʹ���ⲿ�������
	//	bByCoreClient:	Ϊ true ʱ����ͨ�� Core Overlay ������ҳ�����
	//					Ϊ true ʱ�����������ʧ�ܻ�Ϊ false ʱͨ�� szWinExplorer ��ָ������IE���
	//	pOSNavigatorInWinExplorer : ��ȷ��ʹ�� szWinExplorer ָ���ĶԻ���������ʱ��szWinExplorer ���������ⲿ�򿪰�ť�����Ĳ���
	if (strURL.IsEmpty())
		return;
...
{
	// Web browser functionality disabled in Linux cross-compile build
	// (ATL/ActiveX not available with MinGW)
	return false;
}
...
{
	//	�Ի��� pDialogBefore �� pDialogAfter ��ͬһ�������� pDialogBefore �� pDialogAfter ǰʱ���� true
	...
	while (true)
	{
		...
		while (pos)
		{
			...
			{
				//	���ҵ� pDialogBefore������ true
				return true;
			}
			if (pDlgCur == pDialogAfter)
			{
				//	���ҵ� pDialogAfter������ false
				return false;
			}
		}
	}
...
bool CECBaseUIMan::NavigateTWRechargeURL(){
	...
	if (gameUI.bEnableTWRecharge){
		...
		if (!strFormat.IsEmpty()){
			...
			GNET::MD5Hash md5;
			GNET::Octets tokenOctets(strToken, strToken.GetLength());
			md5.Update(tokenOctets);
