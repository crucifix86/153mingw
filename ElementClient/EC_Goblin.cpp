...
bool CECGoblin::Init(int tid, int idModel, int iRefineLvl, CECPlayer* pPlayer)
{
	...
	if(idModel == 1)
	{
		m_idModel = CECGoblin::GOBLIN_MODEL1;		// ��ʾ1��ģ��
		szModelFile = m_pDBEssence->file_model1;
	}
	else if(idModel == 2)
	{
		m_idModel = CECGoblin::GOBLIN_MODEL2;		// ��ʾ2��ģ��
		szModelFile = m_pDBEssence->file_model2;
	}	
	else if(idModel == 3)
	{
		m_idModel = CECGoblin::GOBLIN_MODEL3;		// ��ʾ3��ģ��
		szModelFile = m_pDBEssence->file_model3;
	}	
	else if(idModel == 4)
	{
		m_idModel = CECGoblin::GOBLIN_MODEL4;		// ��ʾ4��ģ��
...
//	g_pGame->GetA3DEngine()->GetA3DSkinMan()->SetReplaceShaderFile("armor.sdr", A3DSkinMan::SHADERREPLACE_REFLECTPREFIX);
	...
	if( m_pModel)
	{
		if(oldModel != m_idModel)
		{
			...
			m_pModel->PlayActionByName("����", 1.0f);
...
//	Tick routine
bool CECGoblin::TickBeforeAnimate(DWORD dwDeltaTime)
{
	if (!m_pModel)
		return true;
...
bool CECGoblin::TickAnimation(DWORD dwDeltaTime)
{
	if (m_pModel)
		m_pModel->TickAnimation(dwDeltaTime);
	return true;
}
bool CECGoblin::TickAfterAnimate(DWORD dwDeltaTime)
{
	if ( !m_pModel )
		return true;
...
bool CECGoblin::Tick(DWORD dwDeltaTime, bool bSkipAnimation )
{
	A3DVECTOR3 vecTargetPos;
	if( !m_pPlayer || !m_pPlayer->GetPlayerModel() || !m_pModel )
		return true;
...
void CECGoblin::PlayRefineActiveEffect()
{
	...
	{
		if(m_iRefineLvl >= 6)
			m_pModel->PlayGfx("��������\\ħ����2.gfx", NULL, 1.0f);
		else
			m_pModel->PlayGfx("��������\\�ɹ���.gfx", NULL, 1.0f);
	}
	else
	{
		if(m_iRefineLvl >= 6)
			m_pModel->RemoveGfx("��������\\ħ����2.gfx", NULL);
		else
			m_pModel->RemoveGfx("��������\\�ɹ���.gfx", NULL);
	}
}
