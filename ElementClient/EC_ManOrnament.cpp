...
CRITICAL_SECTION	g_csAddProvider;
...
m_SoundTab(32)
{
	...
	InitializeCriticalSection(&g_csAddProvider);
}
...
CECOrnamentMan::~CECOrnamentMan()
{
	...
	DeleteCriticalSection(&g_csAddProvider);
}
...
bool CECOrnamentMan::Tick(DWORD dwDeltaTime)
{
	...
	CECParallelTickManager<CECOrnament*> elems;
	...
	if (m_OrnamentTab.size())
	{
		...
		for (; it != m_OrnamentTab.end(); ++it)
		{
			...
			elems.AddElem(pOrnament, dwDeltaTime);
			//pOrnament->Tick(dwDeltaTime);
		}
		elems.Tick(dwDeltaTime);
	}
	...
	if (m_GFXTab.size())
	{
		CECParallelECMGFXTickAnimationManager<A3DGFXEx*> gfxElems;
		GFXTable::iterator it = m_GFXTab.begin();
		for (; it != m_GFXTab.end(); ++it)
		{
			GFXNODE* pNode = *it.value();
			if (pNode->pGfx)
				gfxElems.AddElem(pNode->pGfx, dwDeltaTime);
				//pNode->pGfx->TickAnimation(dwDeltaTime);
		}
		gfxElems.TickAnimation(dwDeltaTime);
		it = m_GFXTab.begin();
		for (; it != m_GFXTab.end(); ++it)
		{
			GFXNODE* pNode = *it.value();
...
void CECOrnamentMan::DeliverLoadedECModels()
{
	...
	for (int i=0; i < m_aLoadedECMs.GetSize(); i++)
	{
		...
		else
		{
			...
			{
				m.pModel->PlayActionByName("����", 1.0f);
			}
