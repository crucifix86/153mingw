...
// id items for Global TownScroll
...
const A3DVECTOR3 DefaultUpMatrix = A3DVECTOR3(0.0f, 1.0f, 0.0f);
...
m_MoveCtrl(this)
{
	...
	m_bCanResurrectWithCash = false;

	// Quick Teleport info
	for (int i = 0; i < FIX_POSITION_TRANSMIT_MAX_POSITION_COUNT; i++)
		m_FixPositionList[i].world_tag = -1;
	m_iFixPositionEnergy = 0;

	// Clear Solo Tower Info (with zeros)
	m_SoloTowerInfo = {};
...
bool CECHostPlayer::Init(const S2C::cmd_self_info_1& Info)
{
	...
	m_pTargetDirGFX = g_pGame->GetGFXCaster()->LoadGFXEx(res_GFXFile(RES_GFX_TARGETDIR));
	m_pFloatDust		= g_pGame->GetGFXCaster()->LoadGFXEx(res_GFXFile(RES_GFX_FLOATING_DUST));
	elementdataman* pDB = g_pGame->GetElementDataMan();
	DATA_TYPE DataType;
	ASTROLABE_APPEARANCE_CONFIG* pDBAppearance = (ASTROLABE_APPEARANCE_CONFIG*)pDB->get_data_ptr(2016, ID_SPACE_CONFIG, DataType);
	for (int i = 0; i < ASTROLABE_ADDON_MAX; i++) {
		AString strFile = pDBAppearance->list[i].file_gfx;
		strFile.CutLeft(4);
		m_pStarChartGlow[i] = g_pGame->GetGFXCaster()->LoadGFXEx(strFile);
	}
	...
	if (Info.state2 & GP_STATE2_MNFACTION_MASK)
	{
		/// TODO
		pData += sizeof(int);
		pData += sizeof(int);
	...
	int vipLevel = 0;
	int vipScore = 0;
	if (Info.state2 & GP_STATE2_VIP_MASK)
	{
		vipLevel = *(int*)pData;
		pData += sizeof(int);
		vipScore = *(int*)pData;
...
void CECHostPlayer::Release()
{
	...
	for (int i=0; i < NUM_SYSMODSETS; i++)
	...
	g_pGame->GetGFXCaster()->ReleaseGFXEx(m_pTargetDirGFX);
	...
	CECQShop::Instance().Clear();
	CECBackShop::Instance().Clear();
	CECVipShop::Instance().Clear();

	for (int i = 0; i < FIX_POSITION_TRANSMIT_MAX_POSITION_COUNT; i++)
		m_FixPositionList[i].world_tag = -1;
	for (int i = 0; i < ASTROLABE_ADDON_MAX; i++) {
		A3DGFXEx* pGfx = m_pStarChartGlow[i];
		
		if (pGfx)
			g_pGame->GetGFXCaster()->ReleaseGFXEx(pGfx);

		m_pStarChartGlow[i] = NULL;
	}
...
bool CECHostPlayer::UseItemInPack(int iPack, int iSlot, bool showMsg/* = true*/)
{
	...
	//  Is item an firework?
	if (pItem->GetClassID() == CECIvtrItem::ICID_FIREWORK2)
	{
		if (GetProfession() == PROF_GHOST && IsInvisible()) // 刺客隐身时无法播放烟花
		{
			if (showMsg) pGameRun->AddFixedMessage(FIXMSG_CANNOT_USE_WHEN_INVISIBLE);
		...
		if (!m_idSelTarget || !ISPLAYERID(m_idSelTarget) || m_idSelTarget == m_PlayerInfo.cid)
		{
		...
		_firework_args fwargs;

		CECElsePlayer* pPlayer = (CECElsePlayer*)g_pGame->GetGameRun()->GetWorld()->GetPlayerMan()->GetPlayer(m_idSelTarget);
		if (!pPlayer)
		{
			// 目标消失
			return false;
		...
		const ACHAR* m_strName = pPlayer->GetName();
		fwargs.is_friend_list = 1;
		fwargs.target_role_id = m_idSelTarget;
		wcscpy(&fwargs.target_username[0], pPlayer->GetName());


		pSession->c2s_CmdUseItemWithArg(0, 1, (BYTE)iSlot, pItem->GetTemplateID(), &fwargs, sizeof(fwargs));
	...
	{
		CECGameUIMan *pGameUI = g_pGame->GetGameRun()->GetUIManager()->GetInGameUIMan();
		if ((pItem->GetTemplateID() != GLOBAL_TOWNSCROLL_ID) && (pItem->GetTemplateID() != GLOBAL_TOWNSCROLL_ID2) && ((pItem->GetTemplateID() != GLOBAL_TOWNSCROLL_ID3))) {
			if (pGameUI && !IsFighting() && !IsGathering())
		{
			...
			if (pGameUI && !IsFighting() && !IsGathering()){
			CDlgNewTransport* pTransport = (CDlgNewTransport*)pGameUI->GetDialog("Win_NewTransportation");
			if(pTransport)
			{
				pTransport->SetSlotUsage(iSlot);
				pTransport->Show(true);
			}
				}
		}
		return true;
								...
								int idSelTarget/* 0 */, int iForceAtk/* -1 */)
{
	...
	else	//	Have to trace selected object before cast skill
	{
		...
		if (CECCastSkillWhenMove::Instance().IsSkillSupported(pSkill->GetSkillID(), this) &&
			...
			if(g_pGame->GetConfigs()->GetGameSettings().bSkillSender){
				if (!idCastTarget || (pSkill->GetType() == CECSkill::TYPE_FLASHMOVE && pSkill->GetRangeType() == CECSkill::RANGE_SLEF)) {
				...
				else if (m_pWorkMan->CanStartWork(CECHPWork::WORK_TRACEOBJECT))
				{
					...
					if (idCastTarget == 0 || idCastTarget == GetCharacterID()) {
						m_pPrepSkill = pSkill;
						CastSkill(idCastTarget, bForceAttack);
					}
					else {
						...
						m_pPrepSkill = pSkill;
						if (!CanTouchTarget(vTargetPos, fTouchRadius, 2) || GetProfession() == PROF_YUEXIAN)
						{
							m_pPrepSkill = NULL;
							...
							if (IsPlayerMoving() || IsAutoMoving()) {
...
bool CECHostPlayer::Render(CECViewport* pViewport, int iRenderFlag/* 0 */)
{
	...
	if (!IsChangingFace() && (ISPLAYERID(m_idSelTarget) || ISNPCID(m_idSelTarget)) && m_idSelTarget != m_PlayerInfo.cid)
	{
		CECWorld* pWorld = g_pGame->GetGameRun()->GetWorld();
		...
		if (pObject)
		{
			if (m_pPateDist) {
				...
				int iWid, iHei;
				ACString strText;
				strText.Format(_AL("%0.1f"), pObject->CalcDist(vPos, true));
				m_pPateDist->SetText(strText, false);
				pViewport->GetA3DViewport()->Transform(vPos,vScrPos);

				g_pGame->GetFont(RES_FONT_TITLE)->GetTextExtent(strText, a_strlen(strText), &iWid, &iHei);
				g_pGame->GetFont(RES_FONT_TITLE)->TextOut(vScrPos.x - (iWid*0.5f),vScrPos.y+15,strText,NAMECOL_GREEN,17, 0, true);

				/*A3DVECTOR3 vDirH = pObject->GetPos() - vPos;
				vDirH.y = 0.0f;
				vDirH.Normalize();
				vPos += g_vAxisY * (m_aabb.Extents.y * 0.15f);
				vPos.x -= vDirH.x / 1.2f;
				vPos.z -= vDirH.z / 1.2f;
				...
				pFont->TextOut3D(strText, pViewport->GetA3DViewport(), vPos, 18, NAMECOL_GREEN);*/
			}
		}
	}
	...
	bool isShapeModelChanged_b = IsShapeModelChanged();
	bool skinShown_b = (GetMajorModel() ? GetMajorModel()->IsSkinShown(SKIN_HEAD_INDEX) : false);

	if (m_pFaceModel &&
		!isShapeModelChanged_b && !bSkipPlayerRender &&
		!skinShown_b)
	{
	...
	if (iRenderFlag == RD_NORMAL)
	{
		...
		if (g_pGame->GetConfigs()->GetShowPosFlag())
		{
			...
			a_sprintf(szMsg, _AL("X:%.3f, Y:%.3f, Z:%.3f  DIR: X:%.3f, Y:%.3f, Z:%.3f"), vPos.x, vPos.y, vPos.z, vDir.x, vDir.y, vDir.z);
...
bool CECHostPlayer::RenderForBooth(CECViewport* pViewport, int iRenderFlag/* 0 */)
{
	...
	if (g_pGame->GetConfigs()->GetShowPosFlag())
	{
		...
		A3DVECTOR3 vPos = GetPos();
		A3DVECTOR3 vDir = GetDir();
		a_sprintf(szMsg, _AL("POS: X:%.3f, Y:%.3f, Z:%.3f  DIR: X:%.3f, Y:%.3f, Z:%.3f"), vPos.x, vPos.y, vPos.z, vDir.x, vDir.y, vDir.z);
		int y = pViewport->GetA3DViewport()->GetParam()->Height - 20;
		vPos += g_vAxisY * (m_aabb.Extents.y * 3.0f);
		g_pGame->GetA3DEngine()->GetSystemFont()->TextOut3D(szMsg, pViewport->GetA3DViewport(), vPos, 0, 0xffffffff);
	}
...
void CECHostPlayer::Revive(const S2C::cmd_player_revive* pCmd)
{
	...
	// Reset variable for Cash Resurrect
	m_bCanResurrectWithCash = false;
}
...
//		m_pLevelUpGFX->SetParentTM(GetAbsoluteTM());
	...
	if (m_pTargetDirGFX) {
		if (!IsChangingFace() && (ISPLAYERID(m_idSelTarget) || ISNPCID(m_idSelTarget)) && m_idSelTarget != m_PlayerInfo.cid && !m_bCameraOnFocus)
		{
			CECObject* pObject = pWorld->GetObject(m_idSelTarget, 1);
			if (pObject)
			{
				if (m_pTargetDirGFX->GetState() == ST_STOP)
					m_pTargetDirGFX->Start();

				A3DMATRIX4 tm = GetAbsoluteTM();
				tm._42 += 0.01f;

				A3DVECTOR3 vDirH = pObject->GetPos() - GetPos();
				vDirH.y = 0.0f;
				vDirH.Normalize();

				m_pTargetDirGFX->SetParentTM(tm);
				m_pTargetDirGFX->SetDirAndUp(vDirH, DefaultUpMatrix);

			}
			else
				m_pTargetDirGFX->Stop();
		}
		else
			m_pTargetDirGFX->Stop();
	}
	...
	bool bShowingStarChartGlow = false;

	if (!IsChangingFace() && (ISPLAYERID(m_idSelTarget)))
	{
		CECObject* pObject = pWorld->GetObject(m_idSelTarget, 1);
		if (pObject)
		{
			...
			if (pElsePlayer)
			{
				newEquip = ((CECElsePlayer*)pObject)->GetEquip64(EQUIPIVTR_ASTROLABE);
				id = newEquip & 0xFFFF;
				level = (newEquip & 0xFFFF0000) >> 16;

			}
			else
			{
				CECIvtrAstrolabe* item = (CECIvtrAstrolabe*)GetEquipment()->GetItem(EQUIPIVTR_ASTROLABE);
				if ( item )
					level = item->GetActiveSlotCount();
			}


			if (level) 
			{
				bShowingStarChartGlow = true;
				m_pSelectedGFX->Stop();
				for (int i = 0; i < ASTROLABE_ADDON_MAX; i++)
				{
					if (m_pStarChartGlow[i] != NULL)
					{
						if (m_pStarChartGlow[i]->GetState() == ST_STOP && i == (level - 1))
						{
							A3DGFXEx* glow = m_pStarChartGlow[level - 1];
							glow->Start();
							glow->SetParentTM(pObject->GetAbsoluteTM());
						}
						else if (m_pStarChartGlow[i]->GetState() != ST_STOP && i == (level - 1))
						{
							// Update Player position maybe
							A3DGFXEx* glow = m_pStarChartGlow[level - 1];
							glow->SetParentTM(pObject->GetAbsoluteTM());
						}
						else if ( i != (level-1))
							m_pStarChartGlow[i]->Stop();
					}
				}
			}
			else
			{
				// player may have unequiped it
				for (int i = 0; i < ASTROLABE_ADDON_MAX; i++)
				{
					if (m_pStarChartGlow[i] != NULL)
						m_pStarChartGlow[i]->Stop();
	...
	if (m_pSelectedGFX && !bShowingStarChartGlow )
	{
		if (!IsChangingFace() && (ISPLAYERID(m_idSelTarget) || ISNPCID(m_idSelTarget)))
		{
			...
			if (pObject)
			{
				...
				A3DMATRIX4 tm = pObject->GetAbsoluteTM();
				tm._42 += 0.01f;

				m_pSelectedGFX->SetParentTM(tm);
			}
			else
				m_pSelectedGFX->Stop();
		}
...
bool CECHostPlayer::CanUseEquipment(CECIvtrEquip* pEquip, int* piReason/* NULL */)
{
	...
	case CECIvtrItem::ICID_ASTROLABE:
	{
...
{
	if (g_pGame->GetConfigs()->GetGameSettings().bSkillSender) {
		if (!m_pPrepSkill || !m_pPrepSkill->ReadyToCast())
		{
		...
		{
			if (m_pPrepSkill && m_pPrepSkill->ChangeToMelee() && !g_pGame->GetConfigs()->GetGameSettings().bAutoAttack) // Do other things like normal attack
			{
				bool bFlag = m_pPrepSkill->ReadyToCast();
				if (m_pWorkMan->IsTracing())
					m_pWorkMan->FinishRunningWork(CECHPWork::WORK_TRACEOBJECT);
...
bool CECHostPlayer::SaveConfigData(void* pDataBuf, int* piSize)
{
	...
	if (pDataBuf)
	{
		for (i = 0; i < SIZE_FASHIONSCSET; i++)
		{
			*((int*)pData) = m_aEquipmentSCSets[i];
...
bool CECHostPlayer::LoadConfigData(const void* pDataBuf)
{
	...
	if (dwVer > 10)
	{
		...
		for (i = 0; i < size; i++)
		{
			m_aEquipmentSCSets[i] = *((int*)pData);
			pData += sizeof(int);
		}
	}
...
bool CECHostPlayer::CanDo(int iThing)
{
	...
	case CANDO_SITDOWN:
		...
		if (IsDead() || IsAboutToDie() || IsJumping() || IsTrading() || IsUsingTrashBox() || 
			IsRooting() || IsReviving() || IsTalkingWithNPC() || IsVipScreenActive() || IsChangingFace() ||
	...
	case CANDO_MOVETO:
	{
		if (IsDead() || IsSitting() || IsTrading() || IsUsingTrashBox() || IsRooting() ||
			IsReviving() || IsTalkingWithNPC() || IsVipScreenActive()  || IsChangingFace() || IsUsingItem() ||
	...
	case CANDO_MELEE:
		...
		if (IsDead() || IsSitting() || !m_idSelTarget || m_idSelTarget == m_PlayerInfo.cid ||
			...
			IsUsingTrashBox() || IsTalkingWithNPC() || IsVipScreenActive() || IsChangingFace() || CannotAttack() ||
	...
	case CANDO_FLY:
		...
		if (IsDead() || IsRooting() || IsSitting() || IsTrading() || IsReviving() || 
			IsUsingTrashBox() || IsVipScreenActive() || IsTalkingWithNPC() || IsChangingFace() || GetBoothState() != 0 ||
	...
	case CANDO_GATHER:
		...
		if (IsDead() || IsAboutToDie() || IsSitting() || IsTrading() || IsUsingTrashBox() ||
			IsReviving() || IsVipScreenActive() || IsTalkingWithNPC() || IsChangingFace() || GetBoothState() != 0 ||
	...
	case CANDO_TRADE:
		...
		if (IsDead() || IsAboutToDie() || IsSitting() || IsJumping() || IsMeleeing() || 
			IsTrading() || IsVipScreenActive() || IsUsingTrashBox() || IsTalkingWithNPC() || IsChangingFace() || 
	...
	case CANDO_SPELLMAGIC:
		if (IsDead() || ISMATTERID(m_idSelTarget) || IsAboutToDie() || IsSitting() ||
			IsJumping() || IsVipScreenActive() || IsFlashMoving() || IsTrading() || IsUsingTrashBox() ||	IsTalkingWithNPC() ||
	...
	{	
		if (IsDead() ||
			...
			IsMeleeing() || IsVipScreenActive() || IsTrading() || IsUsingTrashBox() || IsTalkingWithNPC() ||
	...
	case CANDO_FLASHMOVE:
		...
		if (IsDead() || IsVipScreenActive() || IsAboutToDie() || IsTrading() || IsUsingTrashBox() || IsTalkingWithNPC() ||
	...
	case CANDO_DUEL:
		...
		if (IsDead() || IsAboutToDie() ||IsSitting() || IsFighting() || IsTrading() || 
			IsReviving() || IsVipScreenActive() || IsUsingTrashBox() || IsTalkingWithNPC() || IsChangingFace() ||
...
void CECHostPlayer::RearrangeBoothPacks()
{
	...
	for (int i=0; i < m_pBoothSPack->GetSize(); i++)
	{
	...
	iCnt = 0;
	for (int i=0; i < m_pBoothBPack->GetSize(); i++)
	{
...
//  Get fashion by id & color
CECIvtrEquip* CECHostPlayer::GetEquipByID(int tid, int addoncode, int& index, bool secring)
{
	CECIvtrItem* pItem = NULL;
	int i;
	bool skip = false;
	...
	{
		pItem = m_pPack->GetItem(i);
		if (pItem && pItem->GetTemplateID() == tid)
		{
			if(((CECIvtrEquip*)pItem)->IsEquipment()){
				if (((CECIvtrEquip*)pItem)->GetAddonCode() == (unsigned short)addoncode)
				{
					if(((CECIvtrEquip*)pItem)->GetEquipMask() & EQUIP_MASK64_FINGER1 &&  secring){
						if (skip) {
							index = i;
							return (CECIvtrEquip*)pItem;
						}
						else {
							skip = true;
						}
	...
	if (!pItem)
	{
		//  Check current equipments
		for (i = 0; i < SIZE_ALL_EQUIPIVTR; i++)
		{
			pItem = m_pEquipPack->GetItem(i);
...
// id: high byte is tid, low byte is color
CECIvtrEquip* CECHostPlayer::GetEquipByID(int id, int& index, bool secring)
{
	int tid = (((unsigned int)id) >> 16);
	int addoncode = id & 0x0000ffff;

	return GetEquipByID(tid, addoncode, index, secring);
}
...
//  Set shortcuts for equipments
void CECHostPlayer::SetEquipSCSets(int index, CECIvtrEquip* pEquip)
{
	ASSERT(index >= 0 && index < 240);
	if (!pEquip)
		m_aEquipmentSCSets[index] = 0;
	else
	{
		int tid = pEquip->GetTemplateID();
		int color = pEquip->GetAddonCode();

		// high byte is tid, low byte is color
		m_aEquipmentSCSets[index] = color | (tid << 16);
	}
}
...
//  Equip fashion by suit id
bool CECHostPlayer::EquipEquipmentsBySuitID(int id)
{
	if (id < 0 || id >= 16)
	{
		ASSERT(0);
	...
	//	Check if current equipment suit is empty
	int i = 0;
	for (i = 0; i < 14; i++)
	{
		if (m_aEquipmentSCSets[id * 14 + i] != 0)
			break;
	}
	if (i == 14)
		return false;

	int j, iFashionItemIdx[14], iSlotItemIdx[14], iEmptySlotIdx = 0;
	bool secring = false;
	int firstringslot = 0;
	CECIvtrEquip* pEquip = NULL;
	for (i = 0; i < 14; i++)
	{
		if (m_aEquipmentSCSets[id * 14 + i] == 0) // 快捷栏中无数据
		{
			iFashionItemIdx[i] = 255;
			iSlotItemIdx[i] = 255;
		}
		else	// 快捷栏中有数据
		{
			...
			pEquip = GetEquipByID(m_aEquipmentSCSets[id * 14 + i], iFashionItemIdx[i], ( secring && m_aEquipmentSCSets[id * 14 + i] == m_aEquipmentSCSets[id * 14 + firstringslot]) ? true : false );

			for (int c = 0; c < SIZE_ALL_EQUIPIVTR; c++) {
				if(pEquip){
					if (pEquip->CanEquippedTo(c)) {
					
					
						if (secring && c == EQUIPIVTR_FINGER1)
							iSlotItemIdx[i] = c + 1;
						else
							iSlotItemIdx[i] = c;

						if (c == EQUIPIVTR_FINGER1 && !secring) {
							secring = true;
							firstringslot = i;
						}
						break;
	...
	m_iCurEquipmentSuitID = id;

	//if (m_bAutoFashion)
		//m_AutoFashionCnt.Reset();

	for( i =0 ; i < 14 ; i++){
		if (iFashionItemIdx[i] != 255)
			g_pGame->GetGameSession()->c2s_CmdEquipItem(iFashionItemIdx[i], iSlotItemIdx[i]);
	}
	/*if (iFashionItemIdx[0] != 255)
		g_pGame->GetGameSession()->c2s_CmdEquipItem(iFashionItemIdx[0],EQUIPIVTR_HEAD);
	if (iFashionItemIdx[1] != 255)
		g_pGame->GetGameSession()->c2s_CmdEquipItem(iFashionItemIdx[1], EQUIPIVTR_BODY);
	if (iFashionItemIdx[2] != 255)
		g_pGame->GetGameSession()->c2s_CmdEquipItem(iFashionItemIdx[2], EQUIPIVTR_LEG);
	if (iFashionItemIdx[3] != 255)
		g_pGame->GetGameSession()->c2s_CmdEquipItem(iFashionItemIdx[3], EQUIPIVTR_FOOT);
	if (iFashionItemIdx[4] != 255)
		g_pGame->GetGameSession()->c2s_CmdEquipItem(iFashionItemIdx[4], EQUIPIVTR_WRIST);
	if (iFashionItemIdx[5] != 255)
		g_pGame->GetGameSession()->c2s_CmdEquipItem(iFashionItemIdx[5], EQUIPIVTR_WEAPON);
	if (iFashionItemIdx[6] != 255)
		g_pGame->GetGameSession()->c2s_CmdEquipItem(iFashionItemIdx[6], EQUIPIVTR_NECK);
	if (iFashionItemIdx[7] != 255)
		g_pGame->GetGameSession()->c2s_CmdEquipItem(iFashionItemIdx[7], EQUIPIVTR_WAIST);
	if (iFashionItemIdx[8] != 255)
		g_pGame->GetGameSession()->c2s_CmdEquipItem(iFashionItemIdx[8], EQUIPIVTR_SHOULDER);
	if (iFashionItemIdx[9] != 255)
		g_pGame->GetGameSession()->c2s_CmdEquipItem(iFashionItemIdx[9], EQUIPIVTR_FINGER1);
	if (iFashionItemIdx[10] != 255)
		g_pGame->GetGameSession()->c2s_CmdEquipItem(iFashionItemIdx[10], EQUIPIVTR_FINGER2);
	if (iFashionItemIdx[11] != 255)
		g_pGame->GetGameSession()->c2s_CmdEquipItem(iFashionItemIdx[11], EQUIPIVTR_BIBLE);
	if (iFashionItemIdx[12] != 255)
		g_pGame->GetGameSession()->c2s_CmdEquipItem(iFashionItemIdx[12], EQUIPIVTR_POCKET);
	if (iFashionItemIdx[13] != 255)
		g_pGame->GetGameSession()->c2s_CmdEquipItem(iFashionItemIdx[13], EQUIPIVTR_PROJECTILE);*/
	//g_pGame->GetGameSession()->c2s_CmdEquipFashionItem(iFashionItemIdx[1], iFashionItemIdx[2],
	//iFashionItemIdx[4], iFashionItemIdx[3], iFashionItemIdx[0], iFashionItemIdx[5]);
	return true;
}
...
int CECHostPlayer::AutoSelectTarget()
{
	...
	if (mode == TSL_NORMAL || mode == TSL_MONSTER)
	{
		...
		for (int i=0; i < aCandPlayers.GetSize(); i++)
		{
	...
	if (mode == TSL_NORMAL || mode == TSL_PLAYER)
	{
		...
		for (int i=0; i < aCandNPCs.GetSize(); i++)
		{
...
int CECHostPlayer::GetVipLevelForVipService(VipServices vipService)const
{
	switch (vipService)
	{
	case CVS_SHOPPING:
...
int CECHostPlayer::GetFixPositionEnergy()const
{
	return m_iFixPositionEnergy;
}

int CECHostPlayer::GetCountFixPositionList()const
{
	int c = 0;
	for (int i = 0; i < FIX_POSITION_TRANSMIT_MAX_POSITION_COUNT; i++)
	{
		if (m_FixPositionList[i].world_tag > -1) c++;
	}
	return c;
}
const CECHostPlayer::FIX_POSITION_INFO * CECHostPlayer::GetFixPositionInfoByIndex(int index)
{
	if (index >= 0 && index < FIX_POSITION_TRANSMIT_MAX_POSITION_COUNT) {
		return &m_FixPositionList[index];
	}
}


int CECHostPlayer::GetCashVipFixPositionNum()
{
	if (m_nVipLevel <= 0 || m_nVipLevel > MAX_VIP_LEVEL)
		return 0;
	static int num[MAX_VIP_LEVEL + 1] = { 0, 0, 0, 0, 3 ,6, 10 };
	return num[m_nVipLevel];
}