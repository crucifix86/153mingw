...
void CDlgTaskTrace::FormatTaskTraceDesc(int idTask, bool withName, const ACString& strIndentIn, bool requireActive, int index, bool bCanContributionFinish, int contribution)
{
	...
	CECMatterMan* pMatterMan = GetWorld()->GetMatterMan();
	pMatterMan->FindMattersMineInRegion(matters);
	...
	if (pTemp = pMan->GetTaskTemplByID(idTask))
	{
		...
		if(nANPC > 0 && pTask->CanFinishTask(idTask) && !bSpecialAwardType)
		{
			...
			if( DataType == DT_NPC_ESSENCE )
			{
				...
				// DISABLE OF GFX MONSTER FOR TASK
				for (int j = 0; j < MAX_MONSTER_WANTED; j++)
				{
					...
					for (int k = 0; k < NPCs.GetSize(); k++) {
						if (NPCs[k]->GetTemplateID() == tsi.m_MonsterWanted[j].m_ulMonsterId)
						{
							NPCs[k]->SetRequiredForTask(false);
						}
					}
				}
					...
					if (monid > 0)
					{
						...
						if (pMine)
						{
							for (matIt = matters.begin(); matIt != matters.end(); ++matIt) {
								CECMatter* pMatter = dynamic_cast<CECMatter*>(*matIt);
								if (pMatter->GetTemplateID() == pMine->id) {
									pMatter->SetRequiredForTask(false);
								}
							}
						}
					}
			...
			if (pItem)
			{
				...
				if (pTemp->m_enumMethod != enumTMKillPlayer)
				{
					if(monid > 0)
					{
						...
						// ENABLE OF GFX MONSTER FOR TASK
						for (int k = 0; k < NPCs.GetSize(); k++) {
							if (NPCs[k]->GetTemplateID() == monid)
							{
								NPCs[k]->SetRequiredForTask(true);
							}
						}
					}
					...
					else
					{
						...
						if(pMine)
						{
							...
							for (matIt = matters.begin(); matIt != matters.end(); ++matIt) {
								CECMatter* pMatter = dynamic_cast<CECMatter*>(*matIt);
								if (pMatter->GetTemplateID() == pMine->id) {
									pMatter->SetRequiredForTask(true);
								}
							}
						}
						else
		...
		{
			int nSec = bActiveTask ? max(0, static_cast<int>(tsi.m_ulTimeLimit) - static_cast<int>(tsi.m_ulTimePassed)) : tsi.m_ulTimeLimit;
		...
		if( tsi.m_ulNPCToProtect > 0 )
		{
			...
			int nSec = max(0, static_cast<int>(tsi.m_ulProtectTime) - static_cast<int>(tsi.m_ulTimePassed));
...
void CDlgTaskTrace::RefreshTaskTrace(const std::vector<int> traced, int nTraced, const std::vector<int> titletasks, int num, bool withName)
{
	ShowType showType = GetShowType();
	int size(0);
...
void CDlgTaskTrace::OnCommand_UnTraceTask(const char * szCommand)
{
	...
	if (pObj) {
		...
		RemoveGFXEffectsFromTask(idTask);
	}
}
...
void CDlgTaskTrace::RemoveGFXEffectsFromTask(int taskid) {
	ATaskTemplMan* pMan = g_pGame->GetTaskTemplateMan();
	CECTaskInterface* pTask = GetHostPlayer()->GetTaskInterface();
	...
	ATaskTempl* pTemp = NULL;
	if (pTemp = pMan->GetTaskTemplByID(taskid))
	{
		Task_State_info tsi;
		pTask->GetTaskStateInfo(taskid, &tsi, true);
		...
		for (int j = 0; j < MAX_ITEM_WANTED; j++)
		{
			...
			if (monid > 0)
			{
				...
				const MINE_ESSENCE* pMine = CDlgTask::SearchTaskMine(taskid);
				if (pMine)
				{
					for (matIt = matters.begin(); matIt != matters.end(); ++matIt) {
						CECMatter* pMatter = dynamic_cast<CECMatter*>(*matIt);
...
//		ACString strTask = pTempl->GetName();
		...
		if (hasContributionAward && pEntry) {
			...
			tm tmCur, tmTask;
			ulCurTime -= static_cast<unsigned long>(TaskInterface::GetTimeZoneBias() * 60);
			ulTaskTime -= static_cast<unsigned long>(TaskInterface::GetTimeZoneBias() * 60);
