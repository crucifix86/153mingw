	...
	if (pSvr)
	{
		...
		for(int i=0;i<TASK_STORAGE_LEN;i++)
		{
			...
			if(idTask > 0 && 0 == pTaskInterface->CanDeliverTask(idTask))
			{
				...
				P_AUITREEVIEW_ITEM pItem = pTree->InsertItem( pTmpl ? AWString(pTmpl->GetName()) : strName.Format(_AL("%d"), idTask)  );
		...
		unsigned long lRefreshCount = pStorage->m_StoragesRefreshCount[pSvr->storage_id - 1];//��ǰ���ˢ�µĴ���
		int iRefreshPerDay = GetTaskTemplMan()->GetStorageRefreshPerDay(pSvr->storage_id);//һ������ˢ�µĴ���
		//��ʾ����iRefreshPerDay - lRefreshCount 
