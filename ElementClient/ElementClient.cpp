...
//
///////////////////////////////////////////////////////////////////////////
cxxpool::thread_pool* g_pTickThreadPool;
cxxpool::thread_pool* g_pLoadThreadPool;
...
bool prepare_file_structures()
{
	...
	if( _access(szCachePath, 0) == -1 )
	{
		...
		{
			MessageBoxA(NULL, "�޷�����cacheĿ¼����Ϸ�޷����У�\n��ȷ��Ӳ�����㹻��ʣ��ռ������б���Ϸ��", "����", MB_OK | MB_ICONSTOP);
			return false;
		}
	}
	...
	if( _access(szScreenShotsPath, 0) == -1 )
	{
		...
		{
			MessageBoxA(NULL, "�޷�����screenshotsĿ¼����Ϸ�޷����У�\n��ȷ��Ӳ�����㹻��ʣ��ռ������б���Ϸ��", "����", MB_OK | MB_ICONSTOP);
			return false;
		}
	}
...
bool CheckHardwareCaps(const char *& pszInfo)
{
	...
	{
		MessageBoxA(NULL, "��Ϸ�����⵽���Ļ�������NetMeeting�������У�\n"
			"����NetMeeting����ʱ���������޷�ʹ���Կ�����ά���ٹ��ܣ��������ȹر�NetMeeting ���ٽ�����Ϸ��", 
	...
	if( D3D_OK != hval )
	{
		...
		if( D3D_OK != hval )
		{
			pszInfo = "�޷����16λģʽ����ʾ��ʽ�����������Կ������Լ�DirectX�İ汾";
			return false;
		}
		else
	...
	if( dwLocalTotal < 16 * 1024 * 1024 )
	{
		if( iTexDetail != 2 )
		{
			sprintf(szInfo, "�����Կ����Դ�С��16M����������ͼ����ѡ����[%s]���⽫ʹ������Ϸ�кܿ�������������ͼ���ȵ��ͺ��ٽ�����Ϸ��\n\n�Ƿ���ϵͳ�Զ�ѡ����ʵ���ͼ������������Ϸ��", szTexQuality[iTexDetail]);
			if( IDYES == MessageBoxA(NULL, szInfo, "��ͼ���ȹ���", MB_YESNO | MB_ICONQUESTION) )
			{
	...
	else if( dwLocalTotal < 32 * 1024 * 1024 )
	{
		if( iTexDetail != 2 )
		{
			sprintf(szInfo, "�����Կ�ֻ��32M�Դ棬��������ͼ����ѡ����[%s]���⽫ʹ������Ϸ�кܿ�������������ͼ���ȵ��ͺ��ٽ�����Ϸ��\n\n�Ƿ���ϵͳ�Զ�ѡ����ʵ���ͼ������������Ϸ��", szTexQuality[iTexDetail]);
			if( IDYES == MessageBoxA(NULL, szInfo, "��ͼ���ȹ���", MB_YESNO | MB_ICONQUESTION) )
			{
				g_GameCfgs.GetSystemSettingsPtr()->iTexDetail = 2;
			}
	...
	else if( dwLocalTotal < 64 * 1024 * 1024 )
	{
		if( iTexDetail == 0 )
		{
			sprintf(szInfo, "�����Կ���64M�Դ棬��������ͼ����ѡ����[%s]���⽫ʹ������Ϸ�кܿ�������������ͼ���ȵ��ͺ��ٽ�����Ϸ��\n\n�Ƿ���ϵͳ�Զ�ѡ����ʵ���ͼ������������Ϸ��", szTexQuality[iTexDetail]);
			if( IDYES == MessageBoxA(NULL, szInfo, "��ͼ���ȹ���", MB_YESNO | MB_ICONQUESTION) )
			{
	...
	{
		sprintf(szInfo, "����ϵͳʣ��Ŀ�������ͼ���Դ�С��16M������Ϸ�Ĺ����п��ܻᵼ�»��������������صĴ������Ƿ�Ҫ������Ϸ��");
		if( IDYES != MessageBoxA(NULL, szInfo, "��ͼ���ȹ���", MB_YESNO | MB_ICONQUESTION) )
		{
			pszInfo = "ϵͳʣ��Ŀ�������ͼ���Դ�С��16M���û�ȡ������Ϸ���С�";
			return false;		
		}		
	}
...
CRITICAL_SECTION g_csElementDataIterate;
...
unsigned int g_cpucores = std::thread::hardware_concurrency();
void InitThreadPool()
{
	// may be 0 when not able to detect, so lets use 2 as a default
	// otherwise split the number of cores in half (we dont need them all)
	//g_cpucores = 4;
	...
	g_pTickThreadPool = new cxxpool::thread_pool{ g_cpucores };
	g_pLoadThreadPool = new cxxpool::thread_pool{ g_cpucores };
}
//	WinMain
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, ACHAR* lpCmdLine, int nCmdShow)
{
	...
	if (!CECCommandLine::GetBriefConfig(_AL("nocheckCPU")))
	{
		...
		{
			//OutputDebugString(_AL("AVX2 Supported\n"));
			WCHAR szExeDir[MAX_PATH] = { 0 };
			GetCurrentDirectory(MAX_PATH, szExeDir);

			ShellExecute(NULL, _AL("open"), _AL("elementclient2.exe"), lpCmdLine, szExeDir, SW_SHOWNORMAL);
			return 0;
		}
		else if (InstructionSet::SSE2())
		{
			//OutputDebugString(_AL("SSE2 Supported\n"));
			WCHAR szExeDir[MAX_PATH] = { 0 };
			GetCurrentDirectory(MAX_PATH, szExeDir);

			ShellExecute(NULL, _AL("open"), _AL("elementclient1.exe"), lpCmdLine, szExeDir, SW_SHOWNORMAL);
			return 0;
		}
	}
#endif
	...
	InitializeCriticalSection(&g_csElementDataIterate);
	...
	{
		MessageBoxA(NULL, szInfo, "�Բ�����������ϵͳӲ����ϵͳ���������⣬�޷����б���Ϸ!", MB_OK | MB_ICONSTOP);
#ifdef ENABLE_UNKNOWN_EXIT_CHECK
	...
	{
		MessageBoxA(NULL, "�Բ��������������ϰ�װ����Ϸ��Դȱ����һ����Ҫ���ļ�����Ϸ�޷����У������°�װ��Ϸ�ͻ��ˣ�", szMissedFile, MB_OK | MB_ICONSTOP);
#ifdef ENABLE_UNKNOWN_EXIT_CHECK
		l_bEnableExitCheck = false;
#endif
		return -1;
...
_exit:
	...
	if (g_hDefenceThread){
		::WaitForSingleObject(g_hDefenceThread,5000);	//	2014-12-9 ���ı򣺵ȴ�ʱ����INFINITE�޸�Ϊ5000��Ŀ���Ǳ�����ѭ����
														//	��ѭ����Դ��g_hDefenceThreadͨ��GetWindowTextA���Windows API�ĵ��ã�
														//	GetWindowTextA��ȡ��ǰ���̵Ĵ���ʱ�����䷢��WM_GETTEXT��Ϣ��
														//	����ʱ���߳��Ѿ����ٴ�����Ϣѭ��
	}
	...
	DeleteCriticalSection(&g_csElementDataIterate);
	...
	delete  g_pTickThreadPool;
	delete  g_pLoadThreadPool;
	return 0;
}
...
LRESULT CALLBACK _WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if( g_pGame )
	{
		...
		if (hWnd == g_pGame->GetGameInit().hWnd &&
			...
			b)
		{
			//	�޸� wParam ��ʹ DefWndProc ��ʾΪδʧȥ����
			//	���ڴ�����Ϸ���� WebBrowser �ؼ���ռ���㵼����Ϸ��������ʾΪʧȥ���������
			b = false;
			wParam = TRUE;
		}
	}
