...
bool CECGFXCaster::PlayFW(const char * szFile, int nTimeToFire, const A3DVECTOR3& vecPos, const A3DVECTOR3& vecFaceToDir)
{
	...
	if( 0 )//nTimeToFire > 0 )
	{
		...
		if( m_pCachedFWBody == NULL )
		{
			...
			m_pCachedFWBody->Load("Models\\Matters\\��Ʒ\\�̻�\\�̻�.ecm");
		}
		...
		if( !pFWNode->pFWBody->Load("Models\\Matters\\��Ʒ\\�̻�\\�̻�.ecm") )
		{
			delete pFWNode->pFWBody;
		...
		if( pFWNode->pFWBody )
		{
			pFWNode->pFWBody->PlayActionByName("�̻�ȼ��", 1.0f);
