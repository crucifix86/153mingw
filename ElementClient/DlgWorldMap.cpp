...
static int GetTransmitTargetsNum(const NPC_TRANSMIT_SERVICE *pService)
{
	...
	if (pService)
	{
		...
		num_targets = min(num_targets, static_cast<int>(sizeof(pService->targets)/sizeof(pService->targets[0])));
	}
	return num_targets;
}
...
void CDlgWorldMap::OnEventLButtonUp_Travel(WPARAM wParam, LPARAM lParam, AUIObject *pObj)
{
	...
	if( id >= 0 )
	{
			...
			if (pMgr->UpdateTransWay(pNPC, id))
			{
				...
				else if (transWay.mode == CECMapDlgsMgr::TransWay::TM_NPC_TRANSMIT_DIRECT)
				{
					...
					if (pNPC && pNPC->id_to_discover != 0 && transWayPoints.GetCount()>0)
					{
						...
						aWayPoints.push_back(pNPC->id_to_discover);	//	ѹ�����
		...
		{
			// rune transmit
			CECIvtrItem* pItem = GetHostPlayer()->GetPack()->GetItem(m_iCurrentSlotUse);
			GetGameSession()->c2s_CmdUseItemWithArg(IVTRTYPE_PACK, 1, m_iCurrentSlotUse, pItem->GetTemplateID(), &id, sizeof(id));
			OnCommand_CANCEL("IDCANCEL");

			CDlgNewTransport* pTransport = (CDlgNewTransport*) pGameUI->GetDialog("Win_NewTransportation");
			if (pTransport->IsShow()) {
				pTransport->Show(false);
			}
		}
		else
		{
...
void CDlgWorldMap::BuildTravelMap(DWORD dwType, void* pData, int iMapSelected, int iSlotUse)
{
	// default world id
	int worldid = MAJOR_MAP;
	...
	{
		worldid = iMapSelected;

		// save the worldid
		SetDataPtr((void*)worldid, "WorldID");
		m_iCurrentSlotUse = iSlotUse;
		pMgr->LoadMapTexture(m_pA3DDevice, pImage, CECMapDlgsMgr::MAP_TYPE_GLOBALRUNE, worldid);
	}
	else
	{
		// invalid transmit type
...
// GetPixelsByPos������������ת������Ļ����
// ��֪��	������1024x1024�ķ�����ɣ���Ļ�ϵĵ�ͼ����������a�����飬����b��(�磺�����ͼΪ11x11)
//			��ͼ���ĵ����Ļ����Ϊ(cx,cy)
//			��ͼ���ĵ��Ӧ����������Ϊ(ox,oy)
//          ��ͼͼƬ��������ʾ��ͼ�ķֱ��ʴ�СΪw*h
//			�������ˮƽ������(x,y) (������y����z)
//          ��������ʾ��ͼ�����߶�min(m_nMinSize)
//          �����θ����ĵ�ͼΪ�ֱ���1024*1024��������ͼƬ��Ϊ��Ӧ����1024*768�ķֱ��ʣ��м�768���ȵĲ���Ϊ��ͼ���ݣ�����Ϊ�ڱߣ�
//			������Ҫ�ڵ�ͼ�������߼���װ�Σ�ʵ�ʵ�ͼ�ı߳����Ϊ768
//
// ���:	���λ�ö�Ӧ����Ļ����(sx,sy)
//
// ��ͼ����Ļ��ʵ�ʵĴ�СΪ W*H
...
bool CECMapDlgsMgr::UpdateTransWay(const NPC_ESSENCE* pNPC, int idTarget)
{
	...
	while(!working.empty())
	{
		...
		if(idCur == idSelf)
		{
			...
			m_transWay.mode = TransWay::HasDirectTransmitItem() ? TransWay::TM_NPC_TRANSMIT_DIRECT : TransWay::TM_NPC_TRANSMIT_NORMAL; //	��û��ָ����Ʒ����·��ֻ������ʾ������ʹ����ͨ��ʽ����
...
AString CECMapDlgsMgr::GetMapTexture(MAP_TYPE type, int nMapSelected){	
	AString strFile;
	CECGameRun *pGameRun = g_pGame->GetGameRun();
	int idInstance = pGameRun->GetWorld()->GetInstanceID();
	if( (type == MAP_TYPE_TRANSMIT && idInstance == MAJOR_MAP) || type == MAP_TYPE_SCROLL || type == MAP_TYPE_GLOBALRUNE && nMapSelected == MAJOR_MAP){
		strFile = "InGame\\JourneyMap.dds";
	}
	else if (type == MAP_TYPE_GLOBALRUNE) {

		CECInstance* pInstance = pGameRun->GetInstance(nMapSelected);
...
bool CECMapDlgsMgr::LoadMapTexture(A3DDevice* pDevice, PAUIIMAGEPICTURE pImage, MAP_TYPE type, int nMapSelected)
{
	int idInstance = g_pGame->GetGameRun()->GetWorld()->GetInstanceID();
	AString strFile = GetMapTexture(type, nMapSelected);
...
void CECMapDlgsMgr::DrawAutoMoveFlag(const CECMapDlgsMgr::PosConvertor& conv)
{
	...
	if (CECIntelligentRoute::Instance().IsMoveOn()){		
		...
		int nStep = (int)6/(sumPixelLength/(nTestCount-1));		//	6 �����س� / ÿ����ƽ������ = Ӧ�߹�����
		nStep = a_Max(nStep, 1);
		//	����ͬ������ʾ�ڵ�
...
{
	//	�жϴ� NPC �Ƿ��ǿ����ڴ���ͼ�����ĺϷ� NPC
	return pNPC == NULL
		|| pNPC->id_to_discover == 0		//	����ͼ�ڵ�ͨ�� id_to_discover ��ʶ
		|| pNPC->id == 2218					//	���� NPC ������ NPC �ظ���ʶĳ�����͵㣬������Ƿ����ݣ��������������ң�������ʱ����
		|| pNPC->id == 25843
		|| pNPC->id == 25844;
}
										...
										, A2DSprite *pA2DSpriteMine, A2DSprite *pA2DSpriteMineForMyFaction)
{
	...
	for (CECFactionPVPModel::DomainWithResourceIterator it = begin; it != end; ++ it){
		...
		bool belongToMyFaction = CECDomainGuildInfo::Instance().IsDomainOwner(domain, myFactionID);	//	domain �Ƿ����ڱ���
