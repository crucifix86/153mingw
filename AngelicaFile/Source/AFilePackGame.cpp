...
extern int	AFPCK_GUARDBYTE0CUSTOM;
extern int	AFPCK_GUARDBYTE1CUSTOM;
extern int	AFPCK_MASKDWORDCUSTOM;
extern int	AFPCK_CHECKMASKCUSTOM;

extern int	AFPCK_SAFETAG1CC;
extern int	AFPCK_SAFETAG2CC;
...
bool AFilePackGame::InnerOpen(const char* szPckPath, const char* szFolder, OPENMODE mode, bool bEncrypt, bool bShortName)
{
	...
	case OPENEXIST:
		...
		m_fpPackageFile->m_bUseCustomLoader = strstr(szPckPath,"update") != NULL;
		...
		if (dwVersion == 0x00020003)
		{
			...
			//if( strstr(m_header.szDescription, "File Package") == NULL )
			//	return false;
			...
			if(m_fpPackageFile->m_bUseCustomLoader){
				m_header.dwEntryOffset ^= AFPCK_MASKDWORDCUSTOM;
				if( m_header.guardByte0 != AFPCK_GUARDBYTE0CUSTOM ||
					m_header.guardByte1 != AFPCK_GUARDBYTE1CUSTOM )
				{
					// corrput file
		...
		//		AFERRLOG(("AFilePackGame::Open(), Not enough memory for entries [%s]", szPckPath));
			...
			for (i=0; i < iNumFile; i++)
			{
				...
				m_fpPackageFile->read(&nCompressedSize, sizeof(int), 1);
								if(m_fpPackageFile->m_bUseCustomLoader)
					nCompressedSize ^= AFPCK_MASKDWORDCUSTOM;
				else
				...
				if(m_fpPackageFile->m_bUseCustomLoader)
					nCheckSize = nCheckSize ^ AFPCK_CHECKMASKCUSTOM ^ AFPCK_MASKDWORDCUSTOM;
				else
...
static bool CheckFileEntryValid(AFilePackGame::FILEENTRY* pFileEntry)
{
	...
	{
		// patcher�ڸ���pck��ʱ������������ļ�����Ϊ0���ļ��������һ�������pFileEntry->dwCompressedLength
		// ͨ������¸�ֵΪ0xFFFFFFFC������Ҳ���������������д����������ֵ�����������һ�������С�ļ���Ӧ��̫��
		// ������Ϊ�����FileEntry�Ǵ��� 
		// ��׼������MAX_FILE_PACKAGE���������Ϊ�ǹ���ģ���Ϊpck�ļ����ڸ�ֵʱ���Զ�����������ϲ������κ�һ���ļ��������ֵ
...
bool AFilePackGame::LoadSafeHeader()
{
	...
	if( m_safeHeader.tag1 == 0x4DCA23EF)
		m_bHasSafeHeader = true;
	else
		m_bHasSafeHeader = false;

	if( m_fpPackageFile->m_bUseCustomLoader)
		if( m_safeHeader.tag1 == AFPCK_SAFETAG1CC )
			m_bHasSafeHeader = true;
